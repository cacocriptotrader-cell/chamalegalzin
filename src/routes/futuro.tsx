...
export function WealthProjectionCockpit() {
  ...
  useEffect(() => {
    const feedbackKey = "docfin.feedback.tax_projection.viewed";
    if (typeof sessionStorage !== "undefined" && sessionStorage.getItem(feedbackKey)) return;
    sessionStorage.setItem(feedbackKey, "true");
    const timer = window.setTimeout(() => {
      showMicroFeedbackToast({
        actionId: `tax_projection:${new Date().toISOString().slice(0, 10)}`,
        context: "tax_projection",
        type: "CSAT",
        question: "A projeção fez sentido para você?",
      });
    }, 1200);
