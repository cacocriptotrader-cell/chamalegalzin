...
export interface AccountantClientOperationalState extends OperationalState {
  clientId: string;
}
...
export interface StoreState {
  ...
  workplaces: Workplace[];
  shifts: Shift[];
  doctorOperationalState: OperationalState;
  accountantClientState: AccountantClientOperationalState | null;
...
export interface StoreActions {
  ...
  addShifts: (shifts: Array<NewEntity<Shift>>) => Promise<PersistenceStatus>;
...
async function acceptAccountantInviteInSupabase(doctorId: string): Promise<boolean> {
  const { error } = await supabase.rpc("accept_accountant_invite", { doctor_id: doctorId });
  if (error) {
    logger.error("Falha ao aceitar convite contábil via RPC.", error);
    return false;
  }
  return true;
}
...
async function insertShiftsInSupabase(shifts: Shift[]): Promise<boolean> {
  const userId = await getSupabaseUserId();
  if (!userId) return false;
  const { error } = await supabase.from("shifts").insert(shifts.map((shift) => storeShiftToDb(userId, shift)));
  if (error) {
    logger.error("Falha ao inserir lote de plantões no banco.", error);
    return false;
  }
  return true;
}
...
const initialState: StoreState = {
  ...
  shifts: [],
  doctorOperationalState: {
    workplaces: initialWorkplaces,
    shifts: [],
  },
  accountantClientState: null,
  fixedCosts: [
...
function loadState(): StoreState {
  ...
  try {
    ...
    const parsedDoctorOperationalState = parsed.doctorOperationalState as Partial<OperationalState> | undefined;
    const doctorWorkplaces = Array.isArray(parsedDoctorOperationalState?.workplaces)
      ? parsedDoctorOperationalState.workplaces.map((w: any) => ({
        cutOffDay: 20,
        paymentDay: 5,
        paymentTermDays: 15,
        ...w,
      ...
      : merged.workplaces;
    const doctorShifts = Array.isArray(parsedDoctorOperationalState?.shifts)
      ? parsedDoctorOperationalState.shifts.map((s: any) => normalizeRecordDeductions({
        ...asPJ(s),
        recordStatus: s.recordStatus ?? "consolidated",
        agentNotified: s.agentNotified ?? false,
      }))
      : merged.shifts;
    merged.doctorOperationalState = {
      workplaces: doctorWorkplaces,
      shifts: doctorShifts,
    };
    merged.accountantClientState = null;
...
function saveState(s: StoreState) {
  ...
  try {
    const stateToPersist: StoreState = {
      ...s,
      accountantClientState: null,
    };
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const hydrateFromSupabase = async () => {
      ...
      setState((previous) => ({
        ...
        doctorOperationalState: {
          workplaces: remoteState?.workplaces ?? previous.doctorOperationalState.workplaces,
          shifts: remoteState?.shifts ?? previous.doctorOperationalState.shifts,
        },
        accountantClientState: null,
      }));
    };
  ...
  const actions: StoreActions = {
    ...
    addWorkplace: (w) => {
      ...
      setState((p) => ({
        ...p,
        workplaces: [...p.workplaces, workplace],
        doctorOperationalState: {
          ...p.doctorOperationalState,
          workplaces: [...p.doctorOperationalState.workplaces, workplace],
        },
      }));
      return workplace;
    },
    updateWorkplace: (id, patch) => {
      ...
      setState((p) => ({
        ...p,
        workplaces: p.workplaces.map((w) => (w.id === id ? { ...w, ...patch } : w)),
        doctorOperationalState: {
          ...p.doctorOperationalState,
          workplaces: p.doctorOperationalState.workplaces.map((w) => (w.id === id ? { ...w, ...patch } : w)),
        },
      }));
    },
    removeWorkplace: (id) => {
      ...
      setState((p) => ({
        ...p,
        workplaces: p.workplaces.filter((w) => w.id !== id),
        doctorOperationalState: {
          ...p.doctorOperationalState,
          workplaces: p.doctorOperationalState.workplaces.filter((w) => w.id !== id),
        },
      }));
    },
    addShift: async (s) => {
      ...
      setState((p) => ({
        ...p,
        shifts: [...p.shifts, shift],
        doctorOperationalState: {
          ...p.doctorOperationalState,
          shifts: [...p.doctorOperationalState.shifts, shift],
        },
      }));
      return await syncPromise ? "synced" : "local";
    },
    addShifts: async (shiftInputs) => {
      const shifts = shiftInputs.map((shiftInput) => {
        const wp = state.workplaces.find((w) => w.id === shiftInput.workplaceId);
        const expectedPaymentDate = wp ? fmtISO(calculateExpectedPaymentDate(shiftInput.date, wp)) : undefined;
        const finalExpectedPaymentDate = shiftInput.expectedPaymentDate ?? expectedPaymentDate;
        return {
          id: uuid(),
          recordStatus: "consolidated",
          ...shiftInput,
          paymentStatus: shiftInput.recordStatus === "draft" ? shiftInput.paymentStatus : (shiftInput.paymentStatus ?? "PENDING"),
          expectedPaymentDate: finalExpectedPaymentDate,
          projectedPaymentDate: shiftInput.projectedPaymentDate ?? finalExpectedPaymentDate,
          agentNotified: shiftInput.agentNotified ?? false,
          entityDomain: "PJ",
        } satisfies Shift;
      });

      if (shifts.length === 0) return "local";

      const syncPromise = insertShiftsInSupabase(shifts);
      setState((previous) => ({
        ...previous,
        shifts: [...previous.shifts, ...shifts],
        doctorOperationalState: {
          ...previous.doctorOperationalState,
          shifts: [...previous.doctorOperationalState.shifts, ...shifts],
        },
      }));
      return await syncPromise ? "synced" : "local";
    },
    updateShift: (id, patch) => {
      ...
      setState((p) => ({
        ...
        doctorOperationalState: {
          ...p.doctorOperationalState,
          shifts: p.doctorOperationalState.shifts.map((s) => {
            if (s.id !== id) return s;
            const next = { ...s, ...patch };
            const wp = p.doctorOperationalState.workplaces.find((w) => w.id === next.workplaceId);
            const hasManualExpectedPayment = typeof patch.expectedPaymentDate === "string" && patch.expectedPaymentDate.length > 0;
            if (hasManualExpectedPayment) {
              next.projectedPaymentDate = patch.projectedPaymentDate ?? patch.expectedPaymentDate;
            } else if (wp) {
      ...
      if (currentShift && optimisticShift) {
        void updateShiftInSupabase(optimisticShift).then((synced) => {
          ...
          setState((p) => ({
            ...
            doctorOperationalState: {
              ...p.doctorOperationalState,
              shifts: p.doctorOperationalState.shifts.map((shift) => (shift.id === id ? currentShift : shift)),
            },
          }));
        });
      }
    ...
    batchConsolidateShifts: (ids, consolidationData) => {
      ...
      setState((p) => {
        const selected = new Set(ids);
        const consolidateShift = (shift: Shift): Shift => {
        ...
        return {
          ...p,
          shifts: p.shifts.map(consolidateShift),
          doctorOperationalState: {
            ...p.doctorOperationalState,
            shifts: p.doctorOperationalState.shifts.map(consolidateShift),
          },
        };
      });
    ...
    deleteShift: async (id) => {
      ...
      setState((p) => ({
        ...p,
        shifts: p.shifts.filter((shift) => shift.id !== id),
        doctorOperationalState: {
          ...p.doctorOperationalState,
          shifts: p.doctorOperationalState.shifts.filter((shift) => shift.id !== id),
        },
      }));
      return "synced";
    },
    ...
    acceptAccountantClientRequest: async (request) => {
      ...
      const doctorSynced = await acceptAccountantInviteInSupabase(request.id);
    ...
    setActiveClient: async (clientId) => {
      ...
      setState((p) => ({
        ...p,
        userProfile: { ...p.userProfile, activeClientShiftId: clientId },
        accountantClientState: p.accountantClientState?.clientId === clientId ? p.accountantClientState : null,
      }));
