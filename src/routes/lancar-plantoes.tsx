...
import { ArrowRight, CheckCircle2, Keyboard, Plus, Save, Trash2 } from "lucide-react";
...
function createEmptyRow(workplaceId = ""): StudioRow {
  return {
    id: rowId(),
    date: todayIso(),
    workplaceId,
...
export function ShiftStudioPage() {
  ...
  const defaultWorkplaceId = store.workplaces[0]?.id ?? "";
  const [rows, setRows] = useState<StudioRow[]>(() => [createEmptyRow(defaultWorkplaceId)]);
  const [saving, setSaving] = useState(false);

  const validRows = useMemo(
    () => rows.filter((row) => row.date && row.workplaceId && row.hours > 0 && getRowGross(row, store.workplaces) > 0),
  ...
  function addRow() {
    const next = createEmptyRow(defaultWorkplaceId);
  ...
  function updateRow(id: string, patch: Partial<StudioRow>) {
    setRows((current) => current.map((row) => {
      ...
      if ((patch.workplaceId || patch.hours !== undefined) && !row.grossTouched && !patch.grossTouched) {
        next.gross = getSuggestedGross(next.workplaceId, next.hours, store.workplaces);
      }
      return next;
    }));
  }
  ...
  function removeRow(id: string) {
    setRows((current) => current.length === 1 ? [createEmptyRow(defaultWorkplaceId)] : current.filter((row) => row.id !== id));
  }
  ...
  async function saveValidRows() {
    ...
    setRows((current) => {
      ...
      return remaining.length > 0 ? remaining : [createEmptyRow(defaultWorkplaceId)];
    });
    setSaving(false);
    ...
    >
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_280px]">
        <div className="premium-card overflow-hidden rounded-2xl">
          ...
          {store.workplaces.length === 0 ? (
            <div className="p-8 text-center">
              <h2 className="text-lg font-semibold text-foreground">Cadastre um hospital primeiro</h2>
              <p className="mx-auto mt-2 max-w-lg text-sm leading-relaxed text-muted-foreground">
                O lançamento em massa depende do cadastro do hospital para sugerir valor/hora, regime padrão e regras de recebimento.
              </p>
              <Link
                to="/empresa"
                className="mt-5 inline-flex min-h-11 items-center justify-center rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground"
              >
                Ir para Hospitais
              </Link>
            </div>
          ...
          <div className="mt-5 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs leading-relaxed text-muted-foreground">
                Os plantões salvos viram pendências no Inbox. A revisão fiscal continua separada, protegendo seus cálculos.
              </p>
            </div>
          </div>
        </aside>
...
const ShiftStudioRow = memo(function ShiftStudioRow({
  ...
  return (
    <tr className="transition hover:bg-primary/[0.035]" onKeyDown={handleKeyDown}>
      ...
      <td className="px-3 py-3">
        <select value={row.workplaceId} onChange={(event) => onChange({ workplaceId: event.target.value })} className={cellInputCls}>
...
function getSuggestedGross(workplaceId: string, hours: number, workplaces: Array<{ id: string; hourlyRate: number }>): number {
  const workplace = workplaces.find((item) => item.id === workplaceId);
  return Math.max(0, (workplace?.hourlyRate ?? 0) * (hours || 0));
}

function getRowGross(row: StudioRow, workplaces: Array<{ id: string; hourlyRate: number }>): number {
  return row.grossTouched ? row.gross : getSuggestedGross(row.workplaceId, row.hours, workplaces);
}
