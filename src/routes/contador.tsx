...
function ContadorPage() {
  ...
  const draftShiftsCount = useMemo(
    () => store.shifts.filter((shift) => (shift.recordStatus ?? "consolidated") === "draft").length,
    [store.shifts],
  );
  const accountingReadyCount = paidShifts.length;
  ...
  return (
    <div className="space-y-5 px-5 py-5">
      ...
      <section className="premium-card rounded-2xl p-5">
        ...
        <OperationalSummary
          hasClient={Boolean(activeClient)}
          loading={clientLoading}
          draftCount={draftShiftsCount}
          accountingReadyCount={accountingReadyCount}
        />
...
function OperationalSummary({
  ...
  loading,
  draftCount,
  accountingReadyCount,
}: {
  hasClient: boolean;
  loading: boolean;
  draftCount: number;
  accountingReadyCount: number;
}) {
  return (
    <div className="mt-4 rounded-2xl border border-border bg-surface/60 px-4 py-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Resumo Operacional</p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            {hasClient
              ? "A tabela abaixo continua mostrando apenas dados prontos para conferência contábil."
              : "Selecione um cliente para visualizar o estágio dos registros operacionais."}
          </p>
        </div>

        <div className="grid gap-2 sm:grid-cols-2">
          <MetricPill
            label="Rascunhos / Em Análise"
            value={hasClient && !loading ? draftCount : 0}
            tone={hasClient && !loading && draftCount > 0 ? "warning" : "muted"}
          />
          <MetricPill
            label="Consolidados / Recebidos"
            value={hasClient && !loading ? accountingReadyCount : 0}
            tone={hasClient && !loading && accountingReadyCount > 0 ? "success" : "muted"}
          />
        </div>
      </div>
    </div>
...
function MetricPill({ label, value, tone }: { label: string; value: number; tone: "success" | "warning" | "muted" }) {
  const toneClass =
    tone === "success"
  ...
  return (
    <div className={`min-h-12 rounded-xl border px-3 py-2 ${toneClass}`}>
