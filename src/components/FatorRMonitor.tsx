        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground inline-flex items-center gap-1.5">
              {statusClasses.icon}
              Índice de Otimização Tributária
            </p>
            <h2 className="font-display text-xl mt-2">
              {!hasRevenue
                ? "Sem faturamento PJ Simples na janela fiscal"
                : isSafe
                  ? "Estrutura em zona segura para Anexo III"
                  : `Estrutura em atenção. Reforce a média de folha em ${brl2(fatorR.missingProLabore)} para reduzir o risco tributário.`}
            </h2>
            <p className="text-sm text-muted-foreground mt-2 max-w-3xl">
              A leitura compara seu histórico de faturamento dos últimos 12 meses com a média de folha e pró-labore considerada para a meta de 28%.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:min-w-[520px]">
            <Metric label="Histórico de Faturamento" value={brl2(fatorR.accumulatedRevenue)} />
            <Metric label="Alvo 28%" value={brl2(fatorR.targetProLabore)} />
            <Metric label="Média de Folha" value={brl2(fatorR.accumulatedProLabore)} accent={statusClasses.text} />
            <Metric label="Índice Atual" value={`${fatorR.factorPercent.toFixed(1)}%`} accent={statusClasses.text} />
          </div>
        </div>

        <div className="mt-5">
          <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-2">
            <span>Progresso da média de folha até a meta de 28%</span>
        ...
        <div className="mt-5 rounded-2xl border border-border bg-surface-elevated/60 p-4">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-muted-foreground">
              Informe os acumulados dos 11 meses anteriores ao início do uso do DocFin. Esses valores compõem a janela executiva de 12 meses junto com os meses registrados no aplicativo.
            </p>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <label className="space-y-1.5">
              <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Histórico de faturamento anterior</span>
              <input
                type="number"
                min={0}
            ...
            <label className="space-y-1.5">
              <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Média de folha anterior</span>
              <input
                type="number"
                min={0}
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-4">
            <Metric label="Competência atual" value={brl2(fatorR.currentMonthRevenue)} />
            <Metric label="Histórico no app" value={brl2(fatorR.previous11MonthsRevenueFromStore)} />
            <Metric label="Faturamento calibrado" value={brl2(fatorR.setupRevenue)} />
            <Metric label="Folha calibrada" value={brl2(fatorR.setupProLabore)} />
          </div>
        </div>
      </div>
    </section>
