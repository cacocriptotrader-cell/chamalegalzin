...
/**
 * Relatório executivo — Dossiê de Private Banking.
 * Tela com fundo escuro institucional e luzes esmeralda/ciano.
...
export function ExecutiveReport() {
  ...
  const data = useMemo(() => {
    ...
    for (const sh of s?.shifts ?? []) {
      ...
      const origin: Origin = getShiftRegime(s, sh) === "PF" ? "SUS" : "TUSS";
    ...
    const factorR = getRollingFatorRSnapshot(s, y, m + 1).factorPercent;
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto bg-[#050505] text-slate-200 p-8 flex flex-col font-sans border-none overflow-hidden print:scale-[0.85] print:origin-top print:h-[297mm] print:w-full print:overflow-hidden">
      {/* Luzes decorativas */}
      <div className="absolute -top-[200px] -right-[200px] w-[600px] h-[600px] bg-success/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute -bottom-[150px] -left-[150px] w-[500px] h-[500px] bg-cyan-500/5 rounded-full blur-[100px] pointer-events-none" />

      {/* Cabeçalho */}
      <div className="mb-16 border-l-4 border-primary pl-6 z-10 relative">
        <p className="text-success font-bold tracking-[0.3em] text-sm mb-2">
      ...
      <div className="grid grid-cols-3 gap-8 mb-16 z-10 relative">
        <div className="bg-card border border-border rounded-xl p-8">
          ...
          <p className="tabular-nums text-4xl font-bold text-success">
      ...
      <div className="grid grid-cols-3 gap-6 mb-16 z-10 relative">
        <div className="bg-surface-elevated/70 border border-border rounded-lg p-5">
      ...
      <div className="z-10 relative">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b-2 border-border text-muted-foreground uppercase tracking-widest text-xs">
            ...
            {data.rows.map((r, i) => (
              ...
              >
                ...
                <td className="py-4 pr-4">
                  {r.origin === "TUSS" ? (
                    <span className="bg-success/10 text-success text-xs px-2 py-1 rounded">TUSS</span>
                  ) : r.origin === "SCP" ? (
                ...
                <td className="py-4 text-right tabular-nums text-success font-semibold">
                  {brl2(r.net)}
                </td>
              </tr>
            ))}
          ...
          <tfoot>
            <tr className="border-t-2 border-border">
              <td
                colSpan={4}
                className="pt-5 text-xs uppercase tracking-[0.2em] text-muted-foreground"
              >
                Total Consolidado
              </td>
              ...
              <td className="pt-5 text-right tabular-nums text-success font-bold">
                {brl2(totalNet)}
              </td>
            </tr>
          </tfoot>
      ...
      {/* Rodapé */}
      <div className="mt-20 pt-6 border-t border-border flex items-center justify-between text-xs text-muted-foreground z-10 relative">
