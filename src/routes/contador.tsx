...
function ContadorPage() {
  ...
  const obligationRows = useMemo<ObligationRow[]>(
    () => clientRoster.map((client) => {
      ...
      const rbt12Base = taxBase?.rbt12 ?? taxableBase;
    ...
    [clientRoster, payroll12ExcludingCurrentByDoctor, taxBaseByDoctor, taxObligations],
  );
  ...
  async function refreshTaxObligations(referenceMonth = selectedCompetence) {
    ...
    const [{ data: obligationsData, error: obligationsError }, baseMap, payrollMap] = await Promise.all([
      obligationsQuery,
  ...
  async function fetchTaxBasesByDoctor(clientIds: string[], referenceMonth: string) {
    ...
    const { month, year, start, end, rbt12Start } = getCompetenceWindow(referenceMonth);
    ...
    return ((data ?? []) as TaxableShiftRow[]).reduce<Record<string, TaxBaseSnapshot>>((acc, shift) => {
      ...
      const current = acc[shift.user_id] ?? { monthly: 0, rbt12: 0 };
        ...
        <Dialog.Content className="premium-modal fixed left-1/2 top-1/2 z-50 max-h-[88vh] w-[calc(100vw-32px)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl p-0">
          ...
          <div className="space-y-4 p-5">
              ...
              <section className="premium-panel rounded-2xl p-4">
                ...
                <div className="mt-4 rounded-2xl border border-warning/25 bg-warning/10 p-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <p className="font-mono text-2xl font-semibold text-warning tabular-nums">{brl2(calculationMemory.taxResult.systemEstimatedAmount)}</p>
                  </div>
                </div>
            ...
            <label className="block space-y-1.5">
              <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Pró-labore/Folha Declarada no Mês (R$)</span>
              <input
                type="number"
                min={0}
                step="0.01"
                inputMode="decimal"
                value={payrollAmount}
                onChange={(event) => setPayrollAmount(event.target.value)}
                className={`${inputCls} text-right font-mono tabular-nums`}
                placeholder="0,00"
              />
              <p className="text-xs leading-relaxed text-muted-foreground">
                Este valor simula o Fator R acumulado e altera imediatamente o Anexo e o imposto estimado.
              </p>
            </label>
...
function getCalculationMemory(row: ValidatableObligationRow, payrollAmount: number) {
  ...
  const rbt12 = usableNumber(row.obligation.rbt12, row.rbt12Base);
...
function calculateEstimatedTaxForRow(row: ObligationRow, payrollAmount = row.obligation?.payroll_amount ?? 0) {
  const rbt12 = usableNumber(row.obligation?.rbt12, row.rbt12Base);
  return calculateMedicalSimplesTax({
