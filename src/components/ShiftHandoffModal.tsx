...
import { useStore, computeShift, brl2, fmtDate, getRecordPaymentStatus, type PaymentStatus } from "@/lib/store";
import { PaymentStatusBadge, PAYMENT_STATUS_LABELS } from "@/components/PaymentStatusBadge";
import { X, Send, Copy, UserPlus, Repeat, CheckCircle2, AlertTriangle, Trash2 } from "lucide-react";
...
export function ShiftHandoffModal({ shiftId, onClose }: { shiftId: string; onClose: () => void }) {
  ...
  const [paymentStatus, setPaymentStatus] = useState<PaymentStatus>(getRecordPaymentStatus(shift));
  const [actualPaymentDate, setActualPaymentDate] = useState(shift?.actualPaymentDate ?? "");
  ...
  async function copyText() {
    ...
    } catch {/* Mantém o modal funcional mesmo sem acesso à área de transferência. */}
  }
  function updatePaymentStatus(nextStatus: PaymentStatus) {
    setPaymentStatus(nextStatus);
    setPaymentDateError(false);

    if (nextStatus === "PAID" && !actualPaymentDate) {
    ...
    store.updateShift(shiftId, {
      ...
      actualPaymentDate: nextStatus === "PAID" ? actualPaymentDate : undefined,
    });
  }
  function updateActualPaymentDate(nextDate: string) {
    setActualPaymentDate(nextDate);
    setPaymentDateError(false);
    if (paymentStatus === "PAID" && nextDate) {
      store.updateShift(shiftId, { paymentStatus: "PAID", actualPaymentDate: nextDate });
    }
  }
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full sm:max-w-lg max-h-[90vh] overflow-y-auto bg-card border border-border rounded-t-2xl sm:rounded-2xl"
      >
        {/* Cabeçalho */}
            ...
            <>
              ...
              <div className="rounded-md border border-border bg-background/60 p-3 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Status de recebimento</p>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <select
                    ...
                    className="h-9 rounded-md border border-border bg-secondary/60 px-3 text-sm focus:outline-none focus:border-foreground/40"
                  >
                    {(["PENDING", "PAID", "OVERDUE", "DEFAULTED"] as PaymentStatus[]).map((statusOption) => (
                      <option key={statusOption} value={statusOption}>{PAYMENT_STATUS_LABELS[statusOption]}</option>
                    ))}
                  </select>
                  {paymentStatus === "PAID" && (
                    <label className="space-y-1.5">
                      <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Data de recebimento</span>
                      <input
                        type="date"
                        required
                        value={actualPaymentDate}
                        onChange={(event) => updateActualPaymentDate(event.target.value)}
                        className={`h-9 w-full rounded-md border bg-secondary/60 px-3 text-sm focus:outline-none ${paymentDateError ? "border-rose-400 focus:border-rose-300" : "border-border focus:border-foreground/40"}`}
                      />
                    </label>
                  )}
                </div>
...
{message}
          ...
          <div className="border-t border-border pt-4">
            <button
              ...
              onClick={deleteCurrentShift}
              disabled={deleting}
              className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-destructive/35 bg-destructive/10 px-4 text-sm font-semibold text-destructive transition hover:border-destructive/60 hover:bg-destructive/15 disabled:cursor-wait disabled:opacity-60"
            >
              <Trash2 className="h-4 w-4" />
              {deleting ? "Excluindo plantão..." : "Excluir plantão"}
            </button>
          </div>
        </div>
      </div>
