...
export type ShiftPaymentStatus = "UNPAID" | "PAID";
...
export interface StoreActions {
  ...
  addWorkplace: (w: Omit<Workplace, "id" | "color"> & Partial<Pick<Workplace, "color">>) => void;
...
export const HOSPITAL_COLOR_PALETTE = [
  "#8FAF9A",
  "#8EA7BF",
  "#B8A47E",
  "#A88EA8",
...
export function hospitalColorAt(index: number) {
  return HOSPITAL_COLOR_PALETTE[Math.abs(index) % HOSPITAL_COLOR_PALETTE.length];
}
...
const initialState: StoreState = {
  ...
  workplaces: [
    { id: uid(), name: "Hospital São Lucas", address: "Av. Paulista, 1000", color: hospitalColorAt(0), lat: -23.561, lng: -46.656, regime: "PJ_SIMPLES", hourlyRate: 280, paymentRule: "FIFTH_BUSINESS_DAY", cutOffDay: 20, paymentDay: 5, paymentTermDays: 15 },
    { id: uid(), name: "UPA Leste", address: "R. das Flores, 200", color: hospitalColorAt(1), lat: -23.541, lng: -46.611, regime: "PF", hourlyRate: 220, paymentRule: "TENTH_DAY", cutOffDay: 25, paymentDay: 10, paymentTermDays: 15 },
  ],
  shifts: [],
  fixedCosts: [
...
function loadState(): StoreState {
  ...
  try {
    ...
    merged.workplaces = (merged.workplaces || []).map((w: any, index: number) => ({
      cutOffDay: 20,
      paymentDay: 5,
      paymentTermDays: 15,
      ...w,
      color: isValidHexColor(w?.color) ? w.color : hospitalColorAt(index),
    }));
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const actions: StoreActions = {
    ...
    addWorkplace: (w) => setState((p) => ({
      ...p,
      workplaces: [
        ...p.workplaces,
        { id: uid(), ...w, color: isValidHexColor(w.color) ? w.color : hospitalColorAt(p.workplaces.length) },
      ],
    })),
    updateWorkplace: (id, patch) =>
      setState((p) => {
        const workplaces = p.workplaces.map((w) => (w.id === id ? { ...w, ...patch } : w));
        return {
          ...p,
          workplaces,
          shifts: p.shifts.map((s) => {
            if (s.workplaceId !== id) return s;
            const wp = workplaces.find((w) => w.id === s.workplaceId);
            if (!wp) return s;
            const expectedPaymentDate = fmtISO(calculateExpectedPaymentDate(s.date, wp));
            return { ...s, expectedPaymentDate, projectedPaymentDate: expectedPaymentDate };
          }),
        };
      }),
