...
export function calculateMedicalSimplesTaxStrict(input: MedicalSimplesTaxInput): MedicalSimplesTaxResult {
  ...
  const fatorRBps = rbt12Cents > 0
    ? Math.floor((accumulatedPayrollCents * 10_000) / rbt12Cents)
    : 0;
...
export function calculateRbt12ForSimples(input: Rbt12ForSimplesInput): number {
  ...
  const monthsSinceStart = monthDistance(
    new Date(companyStartDate.getFullYear(), companyStartDate.getMonth(), 1),
    referenceDate,
  ) + 1;

  if (monthsSinceStart < 13) {
    const elapsedRevenueCents = sumRevenueWindowIncludingReference(sortedRevenues, referenceDate, monthsSinceStart);
    return centsToReais(Math.round((elapsedRevenueCents / monthsSinceStart) * 12));
  }
