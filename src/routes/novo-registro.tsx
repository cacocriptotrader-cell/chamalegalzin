...
import {
  ...
  type ShiftTransportMode, type TaxRegime,
} from "@/lib/store";
...
function ShiftForm() {
  ...
  }), [date, workplaceId, originId, hours, suggestedGross, extraCost, transportMode, privateTransportCost]);
  const baseMath = wp ? computeShift(store, preview) : { km: 0, fuelCost: 0, wearCost: 0, tax: 0, net: 0, logistics: 0, settlementAdjustment: 0 };
  ...
  function save() {
    ...
    store.addShift({ 
      ...
      taxRegimeOverride: regimeOverride ?? undefined,
            ...
            return (
              <div className="mt-3 space-y-3">
                <div className={`rounded-xl border p-4 flex gap-3 ${
                  instant ? "border-success/30 bg-success/5"
                  : skipped ? "border-amber-500/30 bg-amber-500/5"
                  : "border-slate-500/30 bg-slate-500/5"
                }`}>
                  <div className="flex-shrink-0 pt-0.5">
                    {instant ? <Zap className="h-5 w-5 text-success" />
                      : skipped ? <AlertTriangle className="h-5 w-5 text-amber-400" />
                      : <CalendarClock className="h-5 w-5 text-slate-400" />}
                  </div>
                  <div className="flex-1 space-y-1">
                    <p className={`text-xs font-semibold uppercase tracking-wider ${
                      instant ? "text-success" : skipped ? "text-amber-400" : "text-slate-300"
                    }`}>
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      {instant 
                        ...
                        : `Este plantão entra no ciclo de corte do dia ${wp.cutOffDay} e tem pagamento previsto para o dia ${wp.paymentDay || 5} do ciclo financeiro configurado.`}
                    </p>
                    {isRPA && (
            ...
            {team.map((m) => (
              <div key={m.id} className="glass-card rounded-2xl p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 px-3 h-12 rounded-xl bg-surface-elevated/40 border border-border">
                    <input type="checkbox" checked={m.isPaid} onChange={(e) => updateMember(m.id, { isPaid: e.target.checked })} className="rounded border-border bg-card text-primary focus:ring-primary" />
                    <span className="text-xs text-muted-foreground">Já pago</span>
                  </div>
                </div>
              </div>
            ))}
            <button onClick={addMember} className="w-full py-4 rounded-2xl border border-dashed border-border text-muted-foreground hover:text-foreground hover:border-border transition flex items-center justify-center gap-2 text-sm">
...
const inputCls = "w-full bg-surface-elevated/60 border border-border rounded-xl h-12 px-4 text-sm focus:outline-none focus:border-primary/40 transition-colors";
