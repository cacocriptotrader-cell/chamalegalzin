...
function ConsolidateShiftForm({
  ...
  const [deductionDescription, setDeductionDescription] = useState(shift.deductionDescription ?? "");
  ...
  function consolidate() {
    ...
    store.updateShift(shift.id, {
      ...
      extraCost: transportMode === "PERSONAL_VEHICLE" ? extraCost : 0,
      deductions,
      deductionDescription: totalDeductions > 0 ? deductionDescription.trim() || undefined : undefined,
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        ...
        {totalDeductions > 0 && (
          <label className="premium-panel block space-y-1.5 rounded-2xl p-3">
            <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Motivo da Dedução</span>
            <input
              value={deductionDescription}
              onChange={(event) => setDeductionDescription(event.target.value)}
              className={dashboardInputCls}
              placeholder="Ex.: Taxa adm + impostos retidos"
            />
            <p className="text-xs leading-relaxed text-muted-foreground">
              Esse texto aparece para o contador como trilha de auditoria do valor abatido.
            </p>
          </label>
        )}
