...
export function WorkplacesCard() {
  ...
  const [manualDistanceKm, setManualDistanceKm] = useState(0);
  ...
  function add() {
    ...
    const usesManualDistance = lat === null || lng === null;
    if (usesManualDistance && (!Number.isFinite(manualDistanceKm) || manualDistanceKm <= 0)) return;
    const finalLat = lat ?? s.base.lat;
    const finalLng = lng ?? s.base.lng;
    s.addWorkplace({
      name: name.trim(),
      address: address.trim() || name.trim(),
      lat: finalLat,
      lng: finalLng,
      manualDistanceKm: usesManualDistance ? manualDistanceKm : undefined,
      regime,
      hourlyRate: 0,
      paymentRule: "",
      cutOffDay,
  ...
  return (
    <Section title="Hospitais (Workplaces)" subtitle="Endereço · imposto · regra de pagamento">
            ...
            <div className="flex gap-3 items-start min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  ...
                  {w.manualDistanceKm && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-warning/10 text-warning inline-flex items-center gap-1">
                      <MapPin className="h-2.5 w-2.5" /> Distância manual: {w.manualDistanceKm} km
                    </span>
                  )}
                </div>
      ...
      <div className="glass-card rounded-2xl p-4 space-y-3">
        ...
        {(lat === null || lng === null) && name.trim().length >= 3 && (
          <div className="rounded-2xl border border-warning/30 bg-warning/10 p-3">
            <Field label="Distância aproximada da sua base até este local (km) *">
              <input
                type="number"
                min={0}
                step="0.1"
                value={manualDistanceKm || ""}
                onChange={(e) => setManualDistanceKm(Math.max(0, +e.target.value || 0))}
                className={inp}
                placeholder="Ex: 8,5"
              />
            </Field>
            <p className="mt-2 text-[10px] leading-relaxed text-warning">
              Se o endereço não aparecer na busca, você pode salvar o hospital com texto livre. A distância manual mantém o cálculo logístico funcionando sem bloquear o cadastro.
            </p>
          </div>
        )}
