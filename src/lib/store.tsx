...
export interface StoreActions {
  ...
  addWorkplace: (w: NewEntity<Workplace>) => void;
...
async function getSupabaseUserId(): Promise<string | null> {
  ...
  if (error || !data.user) {
    console.warn("[Store/Supabase] Usuário não autenticado. Mantendo cache local como fallback.", error);
    return null;
  }
  return data.user.id;
}
...
async function fetchOperationalStateFromSupabase(): Promise<Pick<StoreState, "workplaces" | "shifts"> | null> {
  ...
  if (workplacesResult.error || shiftsResult.error) {
    console.error("[Store/Supabase] Falha ao carregar dados operacionais. Usando cache local.", {
  ...
  console.info("[Store/Supabase] Dados operacionais carregados do banco.", {
...
async function insertWorkplaceInSupabase(workplace: Workplace) {
  ...
  if (error) console.error("[Store/Supabase] Falha ao inserir hospital no banco.", error);
}
...
async function updateWorkplaceInSupabase(workplace: Workplace) {
  ...
  if (error) console.error("[Store/Supabase] Falha ao atualizar hospital no banco.", error);
}
...
async function deleteWorkplaceFromSupabase(id: string) {
  ...
  if (error) console.error("[Store/Supabase] Falha ao remover hospital do banco.", error);
}
...
async function insertShiftInSupabase(shift: Shift): Promise<boolean> {
  ...
  if (error) {
    console.error("[Store/Supabase] Falha ao inserir plantão no banco.", error);
    return false;
  }
  return true;
}

async function updateShiftInSupabase(shift: Shift) {
  const userId = await getSupabaseUserId();
  if (!userId) return;
  ...
  if (error) console.error("[Store/Supabase] Falha ao atualizar plantão no banco.", error);
}

async function deleteShiftFromSupabase(id: string) {
  const userId = await getSupabaseUserId();
  if (!userId) return;
  const { error } = await supabase.from("shifts").delete().eq("id", id).eq("user_id", userId);
  if (error) console.error("[Store/Supabase] Falha ao remover plantão do banco.", error);
}
  ...
  const actions: StoreActions = {
    ...
    updateShift: (id, patch) => {
      ...
      if (currentShift) {
        ...
        void updateShiftInSupabase(nextShift);
      }
      setState((p) => ({
        ...p,
    ...
    batchConsolidateShifts: (ids, consolidationData) => {
      ...
      nextSyncedShifts.forEach((shift) => void updateShiftInSupabase(shift));
      setState((p) => {
        const selected = new Set(ids);
        return {
          ...p,
    ...
    removeShift: (id) => {
      void deleteShiftFromSupabase(id);
