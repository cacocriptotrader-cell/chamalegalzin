...
import { AlertTriangle, ChevronDown, CircleDollarSign, Download, FileSpreadsheet, FileText, Repeat, ShieldCheck, TrendingUp } from "lucide-react";
...
function Fechamento() {
  ...
  const consolidatedShifts = useMemo(
    () => s.shifts
      ...
      .filter((shift) => Boolean(shift.invoiceIssueDate) && isInCompetence(shift.invoiceIssueDate as string, month, year))
      .sort((a, b) => (a.invoiceIssueDate ?? "").localeCompare(b.invoiceIssueDate ?? "") || a.date.localeCompare(b.date)),
    [s.shifts, month, year],
  );
                    ...
                    <div key={shift.id} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5">
                          NF {shift.invoiceIssueDate ? formatShortDate(shift.invoiceIssueDate) : "sem data"} · plantão {formatShortDate(shift.date)} · {shift.hours}h · líquido {brl2(math.net)}
                        </p>
                      </div>
                      <button
                        type="button"
