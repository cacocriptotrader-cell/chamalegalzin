...
import { Building2, Landmark, FileText, Smartphone, Users, Sparkles, ArrowRight, X, type LucideIcon } from "lucide-react";
...
interface FiscalOnboardingModalProps {
  onOpenChange?: (open: boolean) => void;
}

export function FiscalOnboardingModal({ onOpenChange }: FiscalOnboardingModalProps = {}) {
  ...
  function concludeProfile() {
    saveTaxProfile(draft);
    onOpenChange?.(false);
  }

  return (
    <div className="fixed inset-0 z-[80] bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-lg rounded-3xl bg-card border border-border shadow-[0_30px_80px_-20px_rgba(2,6,23,0.8)] overflow-hidden">
        <div className="p-6 md:p-8 border-b border-border">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-2 text-success">
              <Sparkles className="h-4 w-4" />
              <span className="text-[10px] uppercase tracking-[0.25em]">Perfil fiscal</span>
            </div>
            {onOpenChange && (
              <button
                type="button"
                ...
                aria-label="Fechar perfil fiscal"
              >
                <X className="h-4 w-4" />
              </button>
            )}
          </div>
          <h2 className="font-display text-2xl md:text-3xl mt-2 tracking-tight text-foreground">
            ...
            return (
              <div
                ...
                className={`rounded-2xl border p-4 transition backdrop-blur ${
                  s.enabled ? "border-success/40 bg-success/[0.04]" : "border-border bg-surface"
                }`}
              >
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <Icon className={`h-4 w-4 ${s.enabled ? "text-success" : "text-muted-foreground"}`} strokeWidth={1.75} />
                  <span className="text-sm font-medium text-foreground flex-1">{INCOME_SOURCE_LABELS[k]}</span>
                </label>
                {s.enabled && (
                  <div className="mt-3 pl-7 flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent border-b border-border focus:border-primary outline-none py-1.5 text-success tabular-nums text-base"
                    />
        ...
        <div className="p-6 md:p-8 border-t border-border flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Renda global mensal</p>
            <p className="font-display text-2xl text-success tabular-nums">{brl(total)}</p>
          </div>
          <button
            disabled={!anyEnabled}
            onClick={concludeProfile}
            className="inline-flex items-center gap-2 rounded-xl bg-primary hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed text-primary-foreground font-semibold text-sm px-5 py-3 transition"
          >
