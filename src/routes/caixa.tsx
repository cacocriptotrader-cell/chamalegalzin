...
import {
  ...
  brl, brl2, PAYMENT_RULE_LABELS, TAX_LABELS, isConsolidatedRecord, getShiftRegime,
} from "@/lib/store";
...
function CashFlow() {
  ...
  const { groups, total, totalCount } = useMemo(() => {
    ...
    const items: (ProjectedItem & { status: NonNullable<typeof store.shifts[number]["status"]> | "CONFIRMADO"; coveredBy?: string })[] = store.shifts.filter(isConsolidatedRecord).map((s) => {
  ...
  const billingByMonth = useMemo(() => {
    ...
    store.shifts.filter(isConsolidatedRecord).forEach((s) => {
      const wp = store.workplaces.find((w) => w.id === s.workplaceId);
      ...
      cur.items.push({
        ...
        date: s.date,
        hospital: wp?.name ?? "—",
        regimeLabel: wp ? TAX_LABELS[getShiftRegime(store, s)] : "—",
        gross: s.gross,
      });
      map.set(k, cur);
    });
    ...
    const rows = store.shifts
      ...
      .map((s) => {
        ...
        const cells = [
          ...
          wp?.name ?? "—",
          wp ? TAX_LABELS[getShiftRegime(store, s)] : "—",
          String(s.hours),
      ...
      <Section title="Relatórios & Caixa" subtitle="Quando o dinheiro cai — projeção por hospital">
        <div className="glass-card rounded-2xl p-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={exportCSV}
              className="shrink-0 inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-medium border border-border bg-slate-900/60 hover:bg-slate-900/80 text-slate-100 transition backdrop-blur"
      ...
      <Section title="Faturamento por Mês" subtitle="Plantões agrupados pela data de execução · valores brutos">
        <div className="space-y-2">
          {billingByMonth.length === 0 && (
            <div className="rounded-2xl border border-border bg-slate-900/40 backdrop-blur p-8 text-center text-sm text-slate-400">
          ...
          {billingByMonth.map((g) => {
            ...
            return (
              <div key={g.key} className="rounded-2xl border border-border bg-slate-900/40 backdrop-blur overflow-hidden">
                <button
                    ...
                    {g.items.map((it) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left p-3 flex items-start justify-between gap-3 hover:bg-secondary/40 transition"
                        >
                          <div className="min-w-0">
...
function SurgeryLedgerPanels() {
  ...
  const toReceive = store.surgeries.filter(
    (s) => isConsolidatedRecord(s) && s.myRole === "MEMBRO_EQUIPE" && !s.isReceived
  ...
  store.surgeries.filter(isConsolidatedRecord).forEach((s) => {
          ...
          {toReceive.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {toReceive.map((s) => {
                ...
                return (
                  <div key={s.id} className="group relative py-3 flex items-center justify-between gap-3 px-1 -mx-1 rounded-md transition-colors hover:bg-secondary/40">
                    <div className="min-w-0">
                    ...
                    <div className="relative shrink-0 h-8 min-w-[160px] flex items-center justify-end">
                      ...
                      <div className="absolute inset-0 flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 animate-fade-in">
                        ...
                        <a href={wa} target="_blank" rel="noreferrer"
                          className="text-[10px] px-2 py-1 rounded-md bg-success text-success-foreground hover:bg-success/90 transition">
                          Cobrar
                        </a>
                      </div>
                    </div>
              ...
              {pendingRepasses.map((p) => {
                ...
                return (
                  <div key={p.surgeryId + p.member.id}
                       className="group relative py-3 flex items-center justify-between gap-3 px-1 -mx-1 rounded-md transition-colors hover:bg-secondary/40">
                    <div className="min-w-0">
                      <p className="text-sm font-medium truncate inline-flex items-center gap-1">
                    ...
                    <div className="relative shrink-0 h-8 min-w-[180px] flex items-center justify-end">
                      ...
                      <div className="absolute inset-0 flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-200 animate-fade-in">
                        <button onClick={() => markPaid(p.surgeryId, p.member.id)}
                          className="text-[10px] px-2 py-1 rounded-md bg-success text-success-foreground hover:bg-success/90 transition font-medium">
                          Marcar como Pago
                        </button>
                        <a href={wa} target="_blank" rel="noreferrer"
                          className="text-[10px] px-2 py-1 rounded-md bg-secondary/70 text-white hover:bg-secondary transition border border-border">
                          Cobrar
                        </a>
                      </div>
                    </div>
