...
function processarPlantaoExtraido(payload: WhatsAppAiExtractedShiftPayload): WhatsAppWebhookResult {
  ...
  if (!hospital) {
    console.log("Webhook WhatsApp: hospital não resolvido para plantão extraído.");
    return {
      ok: true,
      action: payload.type,
      mutations: [],
...
function processarCliqueBotao(payload: WhatsAppButtonClickPayload): WhatsAppWebhookResult {
  ...
  if (valorNormalizado === "paid" || valorNormalizado === "pago") {
    console.log("Webhook WhatsApp: confirmação de pagamento recebida.");
    return {
      ok: true,
      action: payload.type,
      mutations: [
  ...
  if (valorNormalizado === "outro") {
    console.log("Webhook WhatsApp: usuário solicitou informar hospital manualmente.");
    return {
      ok: true,
      action: payload.type,
      mutations: [],
  ...
  if (payload.field === "hospital") {
    ...
    if (!hospital) {
      console.log("Webhook WhatsApp: botão de hospital não encontrado nos cadastros enviados.");
      return {
        ok: true,
        action: payload.type,
        mutations: [],
  ...
  console.log("Webhook WhatsApp: clique de botão sem regra aplicável.");
  return {
    ok: false,
    action: payload.type,
    mutations: [],
...
export function processWhatsAppWebhookPayload(payload: WhatsAppWebhookPayload): WhatsAppWebhookResult {
  ...
  console.log("Webhook WhatsApp: tipo de payload não suportado.");
  return {
    ok: false,
