...
import { AlertTriangle, ChevronDown, CircleDollarSign, Download, FileSpreadsheet, FileText, Repeat, ReceiptText, Save, ShieldCheck, TrendingUp } from "lucide-react";
    ...
    <>
      ...
      <div className="no-print">
        ...
        <Section
          ...
          subtitle="Dados de nota fiscal e contraparte ficam aqui, longe da captura rápida. Preencha antes de exportar o CSV V2."
        >
          <div className="premium-card rounded-2xl p-4">
            {consolidatedShifts.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-surface/60 p-6 text-center">
                ...
                <p className="mt-3 text-sm font-semibold text-foreground">Nenhum plantão consolidado neste mês.</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                  A revisão fiscal aparece assim que houver registros consolidados para a competência selecionada.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {consolidatedShifts.map((shift) => (
                  <TaxReviewShiftCard key={shift.id} shift={shift} store={s} />
                ))}
              </div>
            )}
          </div>
...
function TaxReviewShiftCard({ shift, store }: { shift: Shift; store: ReturnType<typeof useStore> }) {
  ...
  const hasRepasse = Array.isArray(shift.deductions) && shift.deductions.some((deduction) => deduction.type === "REPASSE" && deduction.amount > 0);
  const missingInvoiceData = !invoiceNumber.trim() || !invoiceIssueDate;
  const missingCounterparty = hasRepasse && !counterpartyDocument.trim();

  useEffect(() => {
    setInvoiceNumber(shift.invoiceNumber ?? "");
    setInvoiceIssueDate(shift.invoiceIssueDate ?? "");
    setCounterpartyDocument(shift.counterpartyDocument ?? "");
    setSaved(false);
  }, [shift.id, shift.invoiceNumber, shift.invoiceIssueDate, shift.counterpartyDocument]);

  function saveTaxData() {
    store.updateShift(shift.id, {
      invoiceNumber: invoiceNumber.trim() || undefined,
      invoiceIssueDate: invoiceIssueDate || undefined,
      counterpartyDocument: counterpartyDocument.trim() || undefined,
    });
    setSaved(true);
  ...
  return (
    <Collapsible.Root open={open} onOpenChange={setOpen} className="rounded-2xl border border-border bg-surface/70">
      <Collapsible.Trigger className="flex min-h-16 w-full items-center justify-between gap-3 px-4 py-3 text-left transition hover:bg-surface-elevated/50">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className="truncate text-sm font-semibold text-foreground">{workplace?.name ?? "Hospital não cadastrado"}</p>
            {(missingInvoiceData || missingCounterparty) ? (
              <span className="rounded-full border border-warning/30 bg-warning/10 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-warning">
                Dados pendentes
              </span>
            ) : (
              <span className="rounded-full border border-success/30 bg-success/10 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-success">
                Pronto para CSV
              </span>
            )}
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            {formatShortDate(shift.date)} · {brl2(shift.gross)} · {hasRepasse ? "contém repasse" : "sem repasse informado"}
          </p>
        </div>
      ...
      <Collapsible.Content className="border-t border-border px-4 py-4">
        <div className="grid gap-3 md:grid-cols-3">
          <label className="space-y-1.5">
            <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Número da NF</span>
            <input
              value={invoiceNumber}
              onChange={(event) => setInvoiceNumber(event.target.value)}
              placeholder="Ex: NF-2026-001"
              className={taxInputCls(missingInvoiceData && !invoiceNumber.trim())}
            />
          </label>
          <label className="space-y-1.5">
            <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Data de Emissão da NF</span>
            <input
              type="date"
              value={invoiceIssueDate}
              onChange={(event) => setInvoiceIssueDate(event.target.value)}
              className={taxInputCls(missingInvoiceData && !invoiceIssueDate)}
            />
          </label>
          <label className="space-y-1.5">
            <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">CPF/CNPJ Contraparte do Repasse</span>
            <input
              value={counterpartyDocument}
              onChange={(event) => setCounterpartyDocument(event.target.value)}
              placeholder={hasRepasse ? "Obrigatório para repasse" : "Opcional"}
              className={taxInputCls(missingCounterparty)}
            />
          </label>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs leading-relaxed text-muted-foreground">
            Esses campos entram no CSV V2 para o contador conciliar competência, emissão de nota e eventual repasse sem ligar para você.
          </p>
          <button
            type="button"
            onClick={saveTaxData}
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90"
          >
...
function taxInputCls(hasWarning: boolean) {
  return `w-full min-h-11 rounded-xl border bg-card px-3 py-2.5 text-base text-foreground outline-none transition placeholder:text-muted-foreground/60 focus:ring-2 ${
    hasWarning
      ? "border-warning/55 focus:border-warning/70 focus:ring-warning/20"
