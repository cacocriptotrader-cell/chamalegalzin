...
function MonthPicker({
  ...
  return (
    <div className="premium-panel rounded-2xl px-3 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
      <p className="mb-2 text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Período analisado</p>
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={() => moveMonth(-1)}
          className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-border text-muted-foreground transition hover:border-primary/40 hover:text-primary"
        ...
        <button
          ...
          className="min-w-[150px] rounded-xl border border-border bg-card px-3 py-2 text-center font-mono text-sm font-semibold capitalize tabular-nums text-foreground transition hover:border-primary/40"
...
function FinancialSummaryWidget({ summary }: { summary: ReturnType<typeof getFinancialSummarySnapshot> }) {
  ...
  const taxesAndExpenses = summary.totalTaxes + summary.monthlyLoggedExpenses;
          ...
          {visibleShifts.length === 0 ? (
            ...
            visibleShifts.map((shift) => (
              <DraftShiftRow
                ...
                onDelete={() => {
                  if (!window.confirm("Excluir este rascunho? Esta ação remove o plantão do Inbox.")) return;
                  store.deleteShift(shift.id);
                  setSelectedIds((current) => current.filter((id) => id !== shift.id));
                  toast.success("Rascunho excluído. Sincronizando em segundo plano.", { duration: 4000 });
                }}
              />
            ))
          )}
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            type="button"
            onClick={onDelete}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-border text-muted-foreground transition hover:border-destructive/40 hover:text-destructive"
...
function ConsolidateShiftForm({ shift, onClose }: { shift: Shift; onClose: () => void }) {
  ...
  function discard() {
    ...
    if (!window.confirm("Excluir esta pendência? Esta ação remove o plantão do Inbox.")) return;
...
function getFinancialSummarySnapshot(store: StoreState, now: Date) {
  ...
  for (const surgery of store.surgeries) {
    ...
    if (surgery.myRole === "TITULAR") {
      ...
      pfWorkRevenue += memberRevenue;
      pfDeductions += Math.abs(calculateTotalDeductions(surgery));
      pfTax += Math.abs(calculateRPAWithholding(memberRevenue, store).total);
    }
  }
  ...
  const totalDeductions = round2(pjDeductions + pfDeductions);
  const totalTaxes = round2(pjTax + pfTax + proLaboreTaxes);
  const consolidatedGross = round2(pjRevenue + pfWorkRevenue);
  const pjNet = round2(Math.max(0, pjRevenue - pjTax - pjDeductions));
  const pfWorkNet = round2(Math.max(0, pfWorkRevenue - pfTax - pfDeductions));
  const pfRevenue = round2(pfWorkRevenue);
  const pfNet = pfWorkNet;
