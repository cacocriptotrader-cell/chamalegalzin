...
export function AppShell() {
  ...
  async function handleLogout() {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        window.localStorage.removeItem("docfin.store.v2");
  ...
  return (
    <div className="premium-shell min-h-dvh overflow-x-hidden text-foreground">
      {/* Navegação superior minimalista com borda única */}
      <header className="sticky top-0 z-40 border-b border-border/80 bg-background/88 pt-safe backdrop-blur-xl supports-[backdrop-filter]:bg-background/78">
        <div className="max-w-7xl mx-auto px-6 md:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-10">
            <div className="flex items-center gap-2">
              ...
              <Link
                to={isAccountant ? "/contador" : "/gestao"}
                className={`hidden border-l border-border pl-2 ml-1 text-[10px] uppercase tracking-[0.18em] transition sm:inline-flex ${
                  isAccountant
                    ? pathname === "/contador"
                      ? "text-foreground"
                      : "text-muted-foreground hover:text-foreground"
                    : moreRoutes.has(pathname)
                      ? "text-foreground"
                      : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {isAccountant ? "Contador" : "Gestão"}
              </Link>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {!isAccountant && (
              <>
                <Link
                  to="/planos"
                  className="hidden min-h-11 items-center gap-1.5 rounded-md border border-border px-3 text-xs font-semibold text-muted-foreground transition-all duration-200 hover:border-primary/50 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 sm:inline-flex"
                >
                ...
                <button
                  ...
                  className="hidden min-h-11 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-semibold text-primary-foreground shadow-[0_0_24px_rgba(15,118,110,0.20)] transition-all duration-200 hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 sm:inline-flex"
                >
            ...
            <button
              ...
              disabled={loggingOut}
              className="inline-flex min-h-11 items-center gap-1.5 rounded-md border border-border px-3 text-xs font-semibold text-muted-foreground transition-all duration-200 hover:border-primary/50 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 disabled:cursor-not-allowed disabled:opacity-55"
            >
      ...
      <main className="mx-auto w-full max-w-7xl overflow-x-hidden px-5 pb-[calc(8rem+env(safe-area-inset-bottom))] sm:px-8 md:px-10 md:pb-20">
        <Outlet />
      </main>

      {/* Barra inferior apenas para mobile */}
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/94 pb-safe shadow-[0_-18px_46px_rgba(1,7,22,0.35)] backdrop-blur md:hidden">
          ...
          {(isAccountant ? accountantNavItems : mobileNavItems.slice(0, 3)).map(({ to, label, icon: Icon }) => {
            ...
            return (
              <Link
                key={to}
                to={to}
                className={`flex min-h-16 flex-col items-center justify-center py-2.5 transition-all duration-200 ${
          ...
          {!isAccountant && (
            <>
              <button
                type="button"
                onClick={() => setQuickCaptureOpen(true)}
                className="relative -mt-5 flex min-h-20 flex-col items-center justify-center text-primary transition-all duration-200"
              >
                <span className="flex h-14 w-14 items-center justify-center rounded-full border border-primary/35 bg-primary text-primary-foreground shadow-[0_0_30px_rgba(15,118,110,0.34)]">
                  <PlusCircle className="h-6 w-6" strokeWidth={2.5} />
                </span>
                <span className="mt-1 text-[10px] font-semibold tracking-tight text-primary">Novo</span>
              </button>
