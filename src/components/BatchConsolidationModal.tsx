...
export function BatchConsolidationModal({ count, open, onOpenChange, onConfirm }: BatchConsolidationModalProps) {
  ...
  const isValid = Boolean(taxationType && paymentStatus);
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="premium-modal fixed left-1/2 top-1/2 z-50 w-[calc(100vw-32px)] max-w-xl -translate-x-1/2 -translate-y-1/2 rounded-xl p-0">
          ...
          <div className="space-y-4 p-5">
            <label className="space-y-1.5 block">
              ...
              <select value={taxationType} onChange={(event) => setTaxationType(event.target.value as TaxRegime | "")} className={inputCls}>
            ...
            {paymentStatus === "PAID" && (
              <label className="space-y-1.5 block">
                ...
                <input type="date" value={actualPaymentDate} onChange={(event) => setActualPaymentDate(event.target.value)} className={inputCls} />
              </label>
            )}
