...
import { Building2, CreditCard, Database, FileText, MailPlus, Settings, ShieldCheck, UserRound, WalletCards } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import { useStore } from "@/lib/store";
import { seedE2ETestData } from "@/lib/e2eSeed2026";
...
function ConfiguracoesHub() {
  ...
  return (
    <div className="px-5 py-5 space-y-5">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start md:self-auto">
          <a
            ...
            className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground transition hover:bg-primary/90"
          >
      ...
      {import.meta.env.DEV && <E2ESeedDeveloperCard />}
      <ProfileBasicsCard />
...
function E2ESeedDeveloperCard() {
  ...
  async function handleRunSeed() {
    ...
    try {
      const result = await seedE2ETestData(store);
      const regimeSummary = Object.entries(result.byRegime)
        .map(([regime, count]) => `${regime}: ${count}`)
        .join(" · ");

      if (result.status === "already_seeded") {
        toast.info(`Seed 2026 já existente: ${result.totalRecords} registros.`, { duration: 5000 });
        return;
      }

      if (result.status === "local") {
        toast.warning(`Seed 2026 aplicado localmente (${result.totalRecords} registros). Verifique a sincronização Supabase.`, {
          duration: 6000,
        });
        console.warn("E2E Seed 2026 persisted locally.", result);
        return;
      }

      toast.success(`Seed 2026 criado: ${result.totalRecords} registros. ${regimeSummary}`, { duration: 6000 });
    } catch (error) {
      console.error("Falha ao executar E2E Seed 2026.", error);
      toast.error(error instanceof Error ? error.message : "Não foi possível executar a seed E2E 2026.", { duration: 6000 });
    } finally {
      setRunning(false);
    }
  }
  ...
  return (
    <section className="premium-card rounded-2xl border-dashed border-primary/25 p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0">
          <p className="inline-flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">
            <Database className="h-3 w-3" />
            QA seed temporária
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            Injeta Jan-Dez/2026 com CLT/Bolsa, PJ Simples e PIX/PF para validar Lei 15.270 e segregação de receitas.
          </p>
        </div>
        <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
          <Link
            to="/auditoria-2026"
            className="inline-flex min-h-11 items-center justify-center rounded-xl border border-border px-4 py-2.5 text-xs font-semibold text-muted-foreground transition hover:border-primary/35 hover:text-primary"
          >
            Ver relatório
          </Link>
          <button
            ...
            disabled={running}
            className="inline-flex min-h-11 items-center justify-center rounded-xl border border-primary/35 bg-primary/10 px-4 py-2.5 text-xs font-semibold text-primary transition hover:bg-primary/15 disabled:cursor-wait disabled:opacity-60"
          >
            {running ? "Executando seed..." : "Run E2E Seed (2026) - Full Regimes"}
          </button>
        </div>
      </div>
    </section>
          ...
          <div className="flex flex-wrap gap-2">
            {specialtySuggestions.slice(0, 4).map((item) => (
              <button
                ...
                className="rounded-full border border-border px-3 py-1.5 text-xs text-muted-foreground transition hover:border-primary/45 hover:text-foreground"
              >
                {item}
              </button>
            ))}
