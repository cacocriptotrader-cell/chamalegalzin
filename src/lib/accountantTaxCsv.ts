...
function buildRows(input: GenerateAccountantTaxCsvInput) {
  ...
  return input.clients.flatMap((client) => {
    ...
    const taxBase = input.taxBaseByDoctor[client.id] ?? {
      monthly: sumShiftGross(clientShifts),
      rbt12: 0,
      monthlyRevenues: buildMonthlyRevenues(clientShifts),
    };
    const rbt12 = calculateRbt12ForSimples({
...
function buildMonthlyRevenues(shifts: AccountantTaxCsvShift[]) {
  const revenueByMonth = shifts.reduce<Map<string, number>>((acc, shift) => {
    if (!shift.invoice_issue_date) return acc;
    const month = referenceMonthFromIsoDate(shift.invoice_issue_date);
    acc.set(month, round2((acc.get(month) ?? 0) + positiveNumber(shift.gross)));
    return acc;
...
function referenceMonthFromIsoDate(isoDate: string) {
  const date = new Date(`${isoDate}T12:00:00`);
