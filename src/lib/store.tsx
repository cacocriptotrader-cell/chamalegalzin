...
import { addMonths, lastDayOfMonth, setDate, parseISO, startOfDay } from "date-fns";
...
export interface Workplace {
  ...
  paymentDay: number;
  /** Compatibilidade: dias aproximados convertidos em ciclos mensais de pagamento (30 = +1 mês, 60 = +2 meses). */
...
function dbShiftToStore(row: any): Shift {
  return normalizeRecordDeductions({
    ...
    counterpartyDocument: row.counterparty_document ?? undefined,
...
function storeShiftToDb(userId: string, shift: Shift) {
  return {
    ...
    counterparty_document: shift.counterpartyDocument ?? null,
...
export function calculateTotalDeductions(record: { deductions?: Deduction[]; settlementAdjustment?: number } | null | undefined): number {
  ...
  if (Array.isArray(record.deductions)) {
    return round2(record.deductions.reduce((sum, deduction) => sum + Math.max(0, deduction.amount || 0), 0));
  }
  return round2(Math.max(0, record.settlementAdjustment || 0));
}
...
export const calculateNetValue = (gross: number, deductions?: Deduction[], legacyAdjustment?: number) => {
  ...
  if (Array.isArray(deductions)) {
    const structuredTotal = deductions.reduce((sum, deduction) => sum + Math.max(0, deduction.amount || 0), 0);
    return round2(base - structuredTotal);
  }
  return round2(base - Math.max(0, legacyAdjustment || 0));
};
...
/**
 * Motor de caixa por ciclo hospitalar.
 *
 * Regra operacional:
 * - Plantões até o dia de corte entram no ciclo corrente.
 * - Plantões após o corte passam para o próximo ciclo.
 * - O pagamento cai no dia fixo de pagamento após o offset mensal configurado.
 *
 * Compatibilidade: `paymentTermDays` continua existindo no schema, mas é usado como
 * aproximação de ciclos mensais: 30 dias = +1 mês, 60 dias = +2 meses.
 */
export function calculateExpectedPaymentDate(
  ...
  const paymentDay = Math.max(1, Math.min(31, workplace.paymentDay || 5));
  const offsetMonths = Math.max(0, Math.ceil(Math.max(0, workplace.paymentTermDays ?? 30) / 30));
  const cycleMonthOffset = sd.getDate() <= cut ? offsetMonths : offsetMonths + 1;
  const paymentMonth = new Date(sd.getFullYear(), sd.getMonth() + cycleMonthOffset, 1);
  const safePaymentDay = Math.min(paymentDay, lastDayOfMonth(paymentMonth).getDate());
  return setDate(paymentMonth, safePaymentDay);
}
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const actions: StoreActions = {
    ...
    removeShift: (id) => {
      const previousShift = state.shifts.find((shift) => shift.id === id);
      setState((p) => ({ ...p, shifts: p.shifts.filter((s) => s.id !== id) }));
      if (previousShift) {
        void deleteShiftFromSupabase(id).then((synced) => {
          if (synced) return;
          setState((p) => {
            if (p.shifts.some((shift) => shift.id === id)) return p;
            return { ...p, shifts: [...p.shifts, previousShift].sort((a, b) => b.date.localeCompare(a.date)) };
          });
        });
      }
    },
