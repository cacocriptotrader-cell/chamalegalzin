...
function ControlBandSection() {
  const cards = [
    {
      title: "Rotina simplificada",
      body: "Plantões, escalas e recebíveis entram no mesmo fluxo operacional, sem planilha paralela.",
      icon: <ScanLine className="h-5 w-5" />,
    },
    {
      title: "Do seu jeito",
      body: "Separe PJ, PIX, CLT e SCP sem misturar caixa pessoal com competência fiscal.",
      icon: <Layers3 className="h-5 w-5" />,
    },
    {
      title: "Compartilhe com segurança",
      body: "O contador acessa relatórios vinculados e auditáveis, sem permissão para corromper plantões.",
      icon: <UsersRound className="h-5 w-5" />,
    },
  ];

  return (
    <section className="bg-primary px-5 py-16 text-primary-foreground md:px-8 md:py-20" aria-labelledby="control-title">
      <div className="mx-auto max-w-7xl">
        <h2 id="control-title" className="mx-auto max-w-3xl text-center text-3xl font-semibold tracking-tight md:text-5xl">
          N possibilidades para ficar no controle da rotina financeira médica.
        </h2>

        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {cards.map((card) => (
            <article key={card.title} className="rounded-2xl border border-white/10 bg-background/20 p-5 shadow-2xl shadow-background/10 backdrop-blur transition hover:-translate-y-1 hover:bg-background/30">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/15 bg-white/10 text-primary-foreground" aria-hidden="true">
                {card.icon}
              </div>
              <h3 className="mt-6 text-xl font-semibold tracking-tight">{card.title}</h3>
              <p className="mt-3 text-sm leading-7 text-primary-foreground/75">{card.body}</p>
            </article>
          ))}
        </div>
      </div>
...
function AssistantExplainerSection() {
  const items = [
    {
      title: "Como o DocFin organiza a escala mensal?",
      body: "O médico seleciona múltiplas datas, reaproveita hospital, valor e duração, e o sistema cria plantões consistentes para auditoria posterior.",
    },
    {
      title: "Como o contador recebe a base?",
      body: "A visão contábil consolida competência fiscal, receita PJ Simples, pró-labore, Fator R e imposto estimado com memória de cálculo.",
    },
    {
      title: "Como a PWA ajuda na rotina?",
      body: "Depois de instalado, o DocFin abre direto da tela inicial e usa cache de assets para carregar mais rápido em ambiente hospitalar.",
    },
  ];

  return (
    <section className="px-5 py-18 md:px-8 md:py-24" aria-labelledby="assistant-title">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-primary">Conheça o cockpit DocFin</p>
          <h2 id="assistant-title" className="mt-5 text-3xl font-semibold tracking-tight text-foreground md:text-5xl">
            Uma experiência de app, com disciplina de backoffice.
          </h2>
          <p className="mt-6 max-w-xl text-base leading-8 text-muted-foreground">
            O visual é simples porque a rotina já é complexa. Cada bloco existe para reduzir fricção operacional ou aumentar confiança contábil.
          </p>
        </div>

        <div className="space-y-3">
          {items.map((item, index) => (
            <details key={item.title} className="group rounded-2xl border border-border bg-card/70 p-4 transition hover:border-primary/45">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-left">
                ...
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border text-primary transition group-open:rotate-45" aria-hidden="true">
                  +
                </span>
              </summary>
              <p className="mt-4 pl-10 text-sm leading-7 text-muted-foreground">{item.body}</p>
            </details>
          ))}
        </div>
      </div>
