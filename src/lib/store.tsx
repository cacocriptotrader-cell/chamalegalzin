...
export interface StoreActions {
  ...
  addShift: (s: NewEntity<Shift>) => void;
...
function loadState(): StoreState {
  ...
  try {
    ...
    merged.shifts = (merged.shifts || []).map((s: any) => normalizeRecordDeductions({ ...asPJ(s), recordStatus: s.recordStatus ?? "consolidated" }));
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const actions: StoreActions = {
    ...
    addWorkplace: (w) => setState((p) => ({ ...p, workplaces: [...p.workplaces, { id: uid(), ...w, entityDomain: "PJ" }] })),
    ...
    batchConsolidateShifts: (ids, consolidationData) => setState((p) => {
