...
function PatrimonioPage() {
  ...
  const processFile = useCallback(
    async (file: File | undefined | null) => {
      ...
      try {
        ...
        if (totalItems === 0) {
          setReadSuccess(true);
          const msg = `PDF lido pelo parser (${DIRPF_PARSER_VERSION}), mas o layout desta declaração ainda não foi reconhecido. Verifique o console para detalhes.`;
          setReadInfo(msg);
          toast.info(msg);
        } else {
          setDraftAssets(parsed.assets.map((asset) => ({ ...asset, previousValue: asset.previousValue ?? 0 })));
          setDraftLiabilities(parsed.liabilities.map((liability) => ({
            ...liability,
            previousValue: liability.previousValue ?? 0,
  ...
  const updateDraftAsset = (id: string, patch: Partial<Pick<DraftAsset, "description" | "previousValue" | "currentValue">>) => {
    setDraftAssets((items) => items.map((item) => item.id === id ? { ...item, ...patch } : item));
  };

  const updateDraftLiability = (
    id: string,
    patch: Partial<Pick<DraftLiability, "description" | "previousValue" | "currentValue">>,
  ) => {
  ...
  return (
    <div className="px-5 py-4 space-y-6">
      ...
      {hasDraftReview && (
        <section className="glass-card rounded-3xl p-5 md:p-6 space-y-5 border border-emerald-300/20">
          ...
          {draftAssets.length > 0 && (
            <div className="space-y-3">
              <p className="text-[10px] uppercase tracking-[0.18em] text-emerald-200/80">Bens & Direitos</p>
              <div className="overflow-x-auto">
                <div className="min-w-[760px] space-y-2">
                  <div className="grid grid-cols-[1fr_150px_150px] gap-3 px-3 text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
                    <span>Código / Descrição</span>
                    <span className="text-right">Valor Ano N-1</span>
                    <span className="text-right">Valor Ano N</span>
                  </div>
                  {draftAssets.map((asset) => (
                    <div key={asset.id} className="grid grid-cols-[1fr_150px_150px] gap-3 rounded-2xl bg-black/20 border border-white/10 p-3 items-center">
                      <div className="min-w-0">
                        ...
                        <input
                          ...
                          className="w-full bg-transparent text-sm font-medium outline-none border-b border-transparent focus:border-emerald-300/50 transition"
                          aria-label="Descrição do bem"
                        />
                      </div>
                      <label className="flex items-center gap-1 rounded-xl bg-slate-950/40 border border-white/10 px-2 py-1.5">
                        <span className="text-[11px] text-muted-foreground">R$</span>
                        <input
                          type="number"
                          min="0"
                          step="0.01"
                          value={toMoneyInput(asset.previousValue)}
                          onChange={(event) => updateDraftAsset(asset.id, { previousValue: parseMoneyInput(event.target.value) })}
                          className="w-full bg-transparent text-right text-sm tabular-nums outline-none"
                          aria-label="Valor do bem no ano anterior"
                        />
                      </label>
                      <label className="flex items-center gap-1 rounded-xl bg-emerald-400/10 border border-emerald-300/20 px-2 py-1.5">
                        <span className="text-[11px] text-emerald-100/70">R$</span>
                        <input
                          type="number"
                          min="0"
                          step="0.01"
                          ...
                          className="w-full bg-transparent text-right text-sm tabular-nums text-emerald-100 outline-none"
                          aria-label="Valor do bem no ano atual"
                        />
                      </label>
                    </div>
                  ))}
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[760px] space-y-2">
                  ...
                  {draftLiabilities.map((liability) => (
                    <div key={liability.id} className="grid grid-cols-[1fr_150px_150px] gap-3 rounded-2xl bg-black/20 border border-white/10 p-3 items-center">
                      <div className="min-w-0">
                        <p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground mb-1">
                      ...
                      <label className="flex items-center gap-1 rounded-xl bg-slate-950/40 border border-white/10 px-2 py-1.5">
                        ...
                        <input
                          ...
                          min="0"
                          step="0.01"
                          value={toMoneyInput(liability.previousValue)}
                          onChange={(event) => updateDraftLiability(liability.id, { previousValue: parseMoneyInput(event.target.value) })}
                          className="w-full bg-transparent text-right text-sm tabular-nums outline-none"
                          aria-label="Valor da dívida no ano anterior"
                        />
                      </label>
                      <label className="flex items-center gap-1 rounded-xl bg-rose-400/10 border border-rose-300/20 px-2 py-1.5">
                        ...
                        <input
                          ...
                          aria-label="Valor da dívida no ano atual"
                        />
                      </label>
                    </div>
                  ))}
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between pt-4 border-t border-white/10">
            <div>
              ...
              <p className="font-display text-3xl text-emerald-200 tabular-nums mt-1">{brl(projectedNetWorth)}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl text-sm font-semibold bg-emerald-300 text-slate-950 hover:bg-emerald-200 transition"
              >
