  ...
  return (
    <div className="px-0 py-5 md:py-7 space-y-5 animate-fade-in">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.22em] text-slate-500">Central de comando</p>
          <h1 className="font-display text-3xl md:text-4xl text-slate-900 mt-1">Hoje, {firstName}</h1>
          <p className="text-sm text-slate-500 mt-2 max-w-2xl">
            Operação médica, backoffice fiscal e vida patrimonial consolidados em uma leitura de elevador.
          </p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <p className="text-[10px] uppercase tracking-[0.18em] text-slate-500">Mês atual</p>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`h-2.5 w-2.5 rounded-full ${event.tone === "emerald" ? "bg-teal-500" : event.tone === "blue" ? "bg-sky-400" : "bg-amber-400"}`} />
                    <p className="text-[10px] uppercase tracking-[0.18em] text-slate-500">
                      {event.kind} · {formatEventDate(event.date)}
                    </p>
                  </div>
                  <h3 className="text-base font-semibold mt-2 truncate text-slate-900">{event.title}</h3>
                  <p className="text-xs text-slate-500 mt-1 line-clamp-2">{event.subtitle}</p>
                </div>
                <p className="font-display text-xl text-teal-600 tabular-nums shrink-0">{brl(event.value)}</p>
              </div>
            </div>
          ))}
        </div>
...
function FinancialSummaryWidget({ summary }: { summary: ReturnType<typeof getFinancialSummarySnapshot> }) {
  const resultTone = summary.consolidatedNet >= 0 ? "text-slate-900" : "text-rose-600";
    ...
    >
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] uppercase tracking-[0.18em] text-slate-500">Dinheiro no bolso</p>
            <p className={`font-display text-4xl mt-2 tabular-nums ${resultTone}`}>
              {brl(summary.consolidatedNet)}
            </p>
            <p className="text-xs text-slate-500 mt-2">Resultado líquido depois de impostos, custos e despesas do mês.</p>
          </div>
        ...
        <div className="mt-5">
          ...
          <div className="h-3 rounded-full bg-slate-900 overflow-hidden border border-slate-200">
            <div className="h-full bg-rose-300" style={{ width: `${expenseRatio}%` }} />
          </div>
...
function WealthWidget({ wealth }: { wealth: ReturnType<typeof getWealthSnapshot> }) {
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-[10px] uppercase tracking-[0.18em] text-slate-500">Patrimônio líquido</p>
        <p className={`font-display text-4xl mt-2 tabular-nums ${wealth.netWorth >= 0 ? "text-teal-600" : "text-rose-600"}`}>
          {brl(wealth.netWorth)}
        </p>
        ...
        {hasWealthData ? (
          <div className="mt-5">
            <div className="h-3 rounded-full bg-slate-100 overflow-hidden flex border border-slate-200">
              <div className="h-full bg-teal-500" style={{ width: `${wealth.assetShare}%` }} />
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="font-display text-xl mt-0.5 text-slate-900">{title}</h2>
          </div>
        </div>
        {action}
      </div>
...
function QuickAction({
  ...
  return (
    <Link
      to={to}
      className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-sky-200 hover:bg-sky-50/40 hover:shadow-md"
    >
    ...
    >
      {shifts.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
          ...
          <h3 className="font-display text-lg mt-3 text-slate-900">Nenhuma pendência</h3>
          <p className="text-sm text-slate-500 mt-1">Rascunhos rápidos aparecerão aqui antes de entrarem nos cálculos fiscais.</p>
        </div>
      ) : (
        <div className="grid gap-3">
          <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <label className="inline-flex items-center gap-2 text-xs font-semibold text-slate-600">
                <input
                  ...
                  className="h-4 w-4 rounded border-slate-300 bg-white accent-slate-900 disabled:opacity-40"
                />
                Selecionar Tudo
              </label>
              <label className="flex flex-col gap-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500 sm:min-w-[220px]">
                Hospital
                <select
                  value={selectedHospitalFilter}
                  onChange={(event) => setSelectedHospitalFilter(event.target.value)}
                  className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs normal-case tracking-normal text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
                >
            ...
            {selectedIds.length > 0 && (
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <span className="text-xs text-slate-500">{selectedIds.length} selecionado{selectedIds.length === 1 ? "" : "s"}</span>
                <button
                  type="button"
          ...
          {visibleShifts.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-5 text-center">
              <p className="text-sm font-semibold text-slate-900">Nenhum rascunho para este hospital</p>
              <p className="mt-1 text-xs text-slate-500">Troque o filtro para visualizar outras pendências.</p>
            </div>
          ) : (
...
function DraftShiftRow({
  ...
  return (
    <div
      className={`rounded-xl border p-4 text-left transition ${active ? "border-sky-300 bg-sky-50 shadow-sm" : "border-slate-200 bg-white shadow-sm hover:border-sky-200 hover:shadow-md"}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          <input
            ...
            className="mt-1 h-4 w-4 rounded border-slate-300 bg-white accent-slate-900"
            aria-label={`Selecionar ${workplace?.name ?? "plantão"}`}
          />
          <button type="button" onClick={onEdit} className="min-w-0 text-left">
            <p className="text-[10px] uppercase tracking-[0.16em] text-slate-500">{formatEventDate(shift.date)} · Rascunho</p>
            <h3 className="mt-1 truncate text-sm font-semibold text-slate-900">{workplace?.name ?? "Hospital não cadastrado"}</h3>
            <p className="mt-1 text-xs text-slate-500">{shift.hours}h · {shift.procedure || "Plantão rápido"}</p>
          </button>
        </div>
        <span className="font-mono text-sm text-slate-900 tabular-nums">{brl2(shift.gross)}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[88vh] w-[calc(100vw-32px)] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-slate-200 bg-white p-0 text-slate-900 shadow-2xl shadow-slate-900/20">
          <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4">
            <div>
              <Dialog.Title className="font-display text-xl text-slate-900">Consolidar pendência</Dialog.Title>
    ...
    >
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Data do plantão</span>
          <input ref={firstFieldRef} type="date" value={date} onChange={(event) => setDate(event.target.value)} className={dashboardInputCls} />
        </label>
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Hospital</span>
          <select
            value={workplaceId}
            onChange={(event) => {
        ...
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Previsão de recebimento</span>
        ...
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Valor bruto</span>
        ...
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Origem</span>
        ...
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Regime fiscal</span>
      ...
      <div className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-2">
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Status de recebimento</span>
          <select
            value={paymentStatus}
            onChange={(event) => {
        ...
        {paymentStatus === "PAID" && (
          <label className="space-y-1.5">
            <span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">Data do recebimento</span>
            <input
              type="date"
              required
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
        <button type="button" onClick={discard} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600">
        ...
        <button
          ...
          onClick={consolidate}
          disabled={!isValid}
          className="rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"
        >
          Consolidar registro
        </button>
      </div>
...
function CardLink({ to, children }: { to: "/" | "/calendario" | "/fechamento" | "/patrimonio"; children: ReactNode }) {
  return (
    <Link to={to} className="text-xs text-sky-600 inline-flex items-center gap-1 transition hover:text-sky-700">
      {children}
...
function MiniMetric({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "neutral" | "success" | "danger" }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
      ...
      <p className={`font-mono text-sm font-semibold mt-1 tabular-nums ${tone === "success" ? "text-teal-600" : tone === "danger" ? "text-rose-600" : "text-slate-900"}`}>
        {value}
      </p>
    </div>
  );
...
function EmptyState({
  ...
  return (
    <div className="flex min-h-[260px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 px-6 text-center">
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500">
        {icon}
      </div>
      <h3 className="font-display text-lg mt-4 text-slate-900">{title}</h3>
      <p className="text-sm text-slate-500 max-w-sm mt-2">{body}</p>
      <Link
        to={to}
...
const dashboardInputCls = "w-full min-h-11 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-base text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20";
