...
export function QuickCaptureModal({ open, onOpenChange, prefill }: QuickCaptureModalProps) {
  ...
  async function saveDraft() {
    ...
    const persistenceTask = store.addShift({
      recordStatus: "draft",
      date,
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          className="premium-modal fixed inset-x-0 bottom-0 z-50 flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-3xl p-0 backdrop-blur sm:left-1/2 sm:top-1/2 sm:bottom-auto sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100vw-32px)] sm:max-w-lg sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl"
        >
          <div className="flex items-start justify-between gap-4 p-5 pb-0 md:p-6 md:pb-0">
            ...
            <Dialog.Close className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-border p-2 text-muted-foreground transition hover:text-foreground" aria-label="Fechar">
              <X className="h-4 w-4" />
            </Dialog.Close>
          </div>
          ...
          <div className="shrink-0 border-t border-border bg-background/95 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] backdrop-blur md:px-6">
            <button
              type="button"
              onClick={saveDraft}
              disabled={!canSave || submitting}
              className="min-h-12 w-full rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground shadow-[0_0_24px_rgba(15,118,110,0.18)] transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground"
            >
...
function TransportButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`min-h-11 rounded-lg px-2 py-2 text-xs font-semibold transition ${
...
const inputCls = "w-full min-h-12 rounded-xl border border-border bg-card px-3 py-2.5 text-base text-foreground outline-none transition placeholder:text-muted-foreground/60 focus:border-primary/60 focus:ring-2 focus:ring-primary/25";
