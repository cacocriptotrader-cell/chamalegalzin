...
function NewShift() {
  ...
  useEffect(() => {
    if (!workplaceId && store.workplaces[0]?.id) {
      setWorkplaceId(store.workplaces[0].id);
    }
  }, [store.workplaces, workplaceId]);
  ...
  async function save(event?: FormEvent<HTMLFormElement>) {
    ...
    const errors = validateBulkShiftForm({
      hasWorkplace: Boolean(wp),
    ...
    try {
      const status = await store.addShifts(sortedDates.map((date) => ({
      ...
      if (status === "local") {
        console.warn("Bulk shift insert returned local persistence. Check Supabase logs if remote sync was expected.");
      }

      nav({ to: "/dashboard" });
    } catch (error) {
      console.error("Erro ao salvar lote de plantões:", error);
      setFormError(error instanceof Error ? error.message : "Não foi possível salvar o lote agora.");
    } finally {
      setSaving(false);
    }
  }
        ...
        <div className="glass-card rounded-2xl p-5 space-y-4">
          <div>
            <span className="text-[11px] uppercase tracking-wider text-muted-foreground flex items-center gap-1 mb-1.5">
              <CalendarDays className="h-3.5 w-3.5" /> Datas do lote
            </span>
      ...
      <Section title="Resumo do dia" subtitle={wp ? TAX_LABELS[wp.regime] : ""}>
        <div className="glass-card rounded-2xl p-5">
          ...
          <div className="border-t border-border pt-4 flex items-end justify-between">
            ...
            <button type="submit" disabled={!wp || sortedDates.length === 0 || saving}
...
function validateBulkShiftForm({
  hasWorkplace,
  selectedDatesCount,
  gross,
  hours,
  ...
  const errors: string[] = [];
  if (!hasWorkplace) errors.push("Selecione um hospital antes de salvar o lote.");
  if (selectedDatesCount < 1) errors.push("Selecione pelo menos uma data.");
  if (!Number.isFinite(gross) || gross <= 0) errors.push("Informe um valor bruto maior que zero.");
  if (!Number.isFinite(hours) || hours <= 0) errors.push("Informe uma duração válida para o plantão.");
  return errors;
}
