  ...
  return (
    <div className="px-0 py-5 md:py-7 space-y-5 animate-fade-in">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="premium-panel rounded-xl px-4 py-3">
    ...
    >
      {events.length === 0 ? (
        ...
        <div className="space-y-3">
          {events.map((event) => (
            <div key={event.id} className="premium-panel rounded-xl p-4">
...
function WealthWidget({ wealth }: { wealth: ReturnType<typeof getWealthSnapshot> }) {
  ...
  return (
    ...
    >
      <div
        className="premium-card overflow-hidden rounded-xl p-5"
        style={{
...
function CockpitCard({
  ...
  return (
    <section className={`premium-card rounded-xl p-5 md:p-6 ${className}`}>
...
function QuickAction({
  ...
  return (
    <Link
      to={to}
      className="premium-card flex items-center justify-between gap-3 rounded-xl p-4 transition hover:border-primary/40 hover:bg-primary/[0.04]"
    >
...
function DraftInbox({ shifts }: { shifts: Shift[] }) {
  ...
  const allSelected = shifts.length > 0 && selectedIds.length === shifts.length;

  useEffect(() => {
    const visibleIds = new Set(shifts.map((shift) => shift.id));
  ...
  function toggleAll() {
    setSelectedIds((current) => current.length === shifts.length ? [] : shifts.map((shift) => shift.id));
  }
    ...
    >
      {shifts.length === 0 ? (
        ...
        <div className="grid gap-3">
          <div className="premium-panel flex flex-col gap-3 rounded-xl p-3 sm:flex-row sm:items-center sm:justify-between">
...
function DraftShiftRow({
  ...
  return (
    <div
      className={`rounded-xl border p-4 text-left transition ${active ? "border-primary/40 bg-primary/10" : "premium-panel hover:border-primary/30"}`}
    >
      <div className="flex items-start justify-between gap-3">
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="premium-modal fixed left-1/2 top-1/2 z-50 max-h-[88vh] w-[calc(100vw-32px)] max-w-3xl -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl p-0">
...
function ConsolidateShiftForm({ shift, onClose }: { shift: Shift; onClose: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="premium-panel grid gap-3 rounded-xl p-4 sm:grid-cols-3">
      ...
      <div className="premium-panel grid gap-3 rounded-xl p-3 md:grid-cols-2">
        <label className="space-y-1.5">
        ...
        {paymentStatus === "PAID" && (
          <label className="space-y-1.5">
            ...
            <input type="date" value={actualPaymentDate} onChange={(event) => setActualPaymentDate(event.target.value)} className={dashboardInputCls} />
          </label>
        )}
      </div>

      <div className="premium-panel mt-3 rounded-xl p-3">
...
function EmptyState({
  ...
  return (
    <div className="flex min-h-[260px] flex-col items-center justify-center rounded-xl border border-dashed border-border/70 bg-card px-6 text-center">
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-border bg-surface-elevated text-muted-foreground">
        {icon}
      </div>
...
const dashboardInputCls = "premium-input";
