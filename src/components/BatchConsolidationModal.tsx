...
export function BatchConsolidationModal({ count, open, onOpenChange, onConfirm }: BatchConsolidationModalProps) {
  ...
  const [invoiceIssueDate, setInvoiceIssueDate] = useState(todayIso());
  ...
  const isValid = Boolean(taxationType && paymentStatus && invoiceIssueDate);
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          className="premium-modal fixed left-1/2 top-1/2 z-50 max-h-[88vh] w-[calc(100vw-32px)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl p-0"
        >
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-1.5 block">
                ...
                <input
                  ...
                  className={`${inputCls} ${invoiceDateError ? "border-rose-400 focus:border-rose-300 focus:ring-2 focus:ring-rose-400/20" : ""}`}
                />
                {invoiceDateError && (
                  <p className="text-xs text-rose-200">Informe a data de emissão da NF para definir a competência fiscal.</p>
                )}
              </label>
              <label className="space-y-1.5 block">
                <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Número da NF (opcional)</span>
                <input value={invoiceNumber} onChange={(event) => setInvoiceNumber(event.target.value)} className={inputCls} placeholder="Ex.: 2026-0012" />
              </label>
            </div>
