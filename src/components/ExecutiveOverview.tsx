...
import {
  useStore, computeShift, monthlyFixedTotal, brl,
  calculateExpectedPaymentDate, getRollingFatorRSnapshot, isConsolidatedRecord,
} from "@/lib/store";
...
export function ExecutiveOverview() {
  ...
  const { netWorth, liquidity30d, factorR, hasPj, wedding } = useMemo(() => {
    ...
    store.shifts.filter(isConsolidatedRecord).forEach((sh) => {
    ...
    const fatorRSnapshot = getRollingFatorRSnapshot(store, now.getFullYear(), now.getMonth() + 1);
    const factorR = fatorRSnapshot.factorPercent;
    const hasPj = fatorRSnapshot.hasRevenue;
  ...
  // Séries determinísticas dos sparklines, sem Math.random no render para manter SSR estável.
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mt-4 mb-2">
      ...
      <Card icon={<Activity className="h-4 w-4" />} label="Liquidez · 30 dias"
        ...
        <p
          className={`font-display text-3xl font-semibold tabular-nums tracking-tight ${
            liquidity30d >= 0 ? "text-success" : "text-rose-500"
          }`}
        >
          {brl(liquidity30d)}
        </p>
      ...
      >
        <div className="flex items-center gap-2 mt-1">
          {fiscalOk ? (
            <ShieldCheck className="h-7 w-7 text-success shrink-0" strokeWidth={2} />
          ) : (
            <ShieldAlert className="h-7 w-7 text-amber-400 shrink-0" strokeWidth={2} />
          )}
          <p className={`font-display text-2xl font-semibold tracking-tight ${
            fiscalOk ? "text-success" : "text-amber-400"
          }`}>
      ...
      <Card icon={<Target className="h-4 w-4" />} label="Meta · Projeto Taubaté"
        ...
        <div className="h-1 rounded-full bg-secondary/60 overflow-hidden mt-3">
          <div
...
function Card({
  ...
  return (
    <div className="relative rounded-xl bg-card/50 border border-border backdrop-blur-md p-4 overflow-hidden">
