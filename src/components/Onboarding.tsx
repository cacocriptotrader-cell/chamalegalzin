  ...
  async function handleNext() {
    if (step === 1) {
      ...
      try {
        ...
        completeOnboarding();
        window.location.href = "/dashboard"; // Force reload to update OnboardingGate state
      } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        ...
        {step === 1 ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <button
              onClick={handleNext}
              className="mt-10 inline-flex items-center gap-2 h-12 px-6 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-all duration-200"
            >
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="space-y-2 mb-8">
              ...
              <p className="text-sm text-muted-foreground">Dados fundamentais para personalização e logística.</p>
            </div>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-card border border-border rounded-xl h-12 px-4 text-sm focus:outline-none focus:border-border transition-colors"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full bg-card border border-border rounded-xl h-12 px-4 text-sm focus:outline-none focus:border-border transition-colors appearance-none"
                >
              ...
              <div className="space-y-2 relative">
                ...
                {openSug && (suggestions.length > 0 || searching) && (
                  <div className="absolute z-20 left-0 right-0 mt-1 bg-card border border-border rounded-xl max-h-48 overflow-auto shadow-2xl">
                    ...
                    {!searching && suggestions.map((sug, i) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 text-xs hover:bg-secondary/60 border-b border-border last:border-0 transition-colors"
                      >
                        <p className="text-muted-foreground truncate">{sug.display_name}</p>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <button
              ...
              className="w-full mt-10 inline-flex items-center justify-center gap-2 h-12 px-6 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 disabled:opacity-20 disabled:cursor-not-allowed transition-all duration-200"
            >
              {saving ? (
        ...
        <p className="text-[11px] text-muted-foreground mt-12 font-mono">
