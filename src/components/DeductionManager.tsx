...
export function DeductionManager({ deductions = [], onChange }: DeductionManagerProps) {
  ...
  return (
    <div className="premium-panel grid gap-3 rounded-xl p-4">
      ...
      {deductions.length > 0 && (
        <div className="divide-y divide-border rounded-xl border border-border bg-card">
          {deductions.map((deduction) => (
            <div key={deduction.id} className="flex flex-col gap-3 px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex shrink-0 items-center justify-between gap-3 sm:justify-end">
                ...
                <button
                  ...
                  className="min-h-11 rounded-lg border border-border px-4 py-3 text-base font-semibold text-muted-foreground transition hover:border-destructive/40 hover:text-destructive"
                >
                  Remover
                </button>
              </div>
      ...
      {isAdding ? (
        <div className="grid gap-3 md:grid-cols-[190px_160px_1fr_auto]">
          ...
          <button
            ...
            disabled={amount <= 0}
            className="min-h-12 rounded-xl border border-border px-5 py-3 text-base font-semibold text-muted-foreground transition hover:border-primary/40 hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
          >
            Adicionar
          </button>
        </div>
        ...
        <button
          ...
          className="min-h-12 rounded-xl border border-border px-5 py-3 text-base font-semibold text-muted-foreground transition hover:border-primary/40 hover:text-primary"
        >
          + Adicionar Desconto
        </button>
      )}
...
const inputCls = "premium-input min-h-12 px-3 py-3 text-base";
