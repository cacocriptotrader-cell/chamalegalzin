...
export const IRRF_ADDITIONAL_REDUCER_LIMIT = 5_000;
export const IRRF_ADDITIONAL_REDUCER_PHASEOUT_LIMIT = 7_350;
export const IRRF_ADDITIONAL_REDUCER_BASE = 978.62;
export const IRRF_ADDITIONAL_REDUCER_FACTOR = 0.133145;
...
export interface ProLaboreResult {
  ...
  readonly additionalReducer: number;
...
export function simulateProLabore(grossAmount: number, dependents: number = 0): ProLaboreResult {
  ...
  /**
   * Lei 15.270/2025: additional IRRF reducer applied from January 2026.
   * The reducer is based exclusively on gross taxable income, not on the
   * optimized IRRF base after deductions.
   */
  const additionalReducerCents = calculateAdditionalIrrfReducerCents(grossCents, irrfAmountCents);
  const finalIrrfAmountCents = Math.max(0, irrfAmountCents - additionalReducerCents);
  const totalWithheldCents = inssAmountCents + finalIrrfAmountCents;
  ...
  return {
    ...
    additionalReducer: fromCents(additionalReducerCents),
    irrfAmount: fromCents(finalIrrfAmountCents),
    totalWithheld: fromCents(totalWithheldCents),
    netAmount: fromCents(netAmountCents),
  };
}

function calculateAdditionalIrrfReducerCents(grossCents: number, irrfAmountCents: number): number {
  if (grossCents <= toCents(IRRF_ADDITIONAL_REDUCER_LIMIT)) {
    return irrfAmountCents;
  }

  if (grossCents <= toCents(IRRF_ADDITIONAL_REDUCER_PHASEOUT_LIMIT)) {
    const grossAmount = fromCents(grossCents);
    const reducer = IRRF_ADDITIONAL_REDUCER_BASE - (IRRF_ADDITIONAL_REDUCER_FACTOR * grossAmount);
    return Math.min(irrfAmountCents, toNonNegativeCents(reducer));
  }
