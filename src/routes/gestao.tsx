...
import { useStore, brl2, brl, TAX_LABELS, monthlyFixedTotal, monthlyFixedIncomeNet, computedProLaboreMonthly, monthlyFixedIncomeGrossTotal, inssCeilingReached, daysUntil, DOCUMENT_KIND_LABELS, DOC_KIND_TO_FIXED_COST, INSS_CEILING, PAYMENT_RULE_LABELS, HOSPITAL_COLOR_PALETTE, calculateCLTNetMonthly, calculateINSSProgressive, calculateIRRF2026, GOAL_CATEGORY_LABELS, type TaxRegime, type PaymentRule, type DocumentKind, type GoalCategory } from "@/lib/store";
import { Section } from "@/components/Section";
import { GoalCard } from "@/components/GoalCard";
import { Plus, Trash2, Building2, Wallet, Car, CalendarClock, Briefcase, Heart, TrendingDown, Receipt, Shield, ShieldAlert, ShieldCheck, Rocket, FileDown, CheckCircle2, MapPin, Search, ChevronDown } from "lucide-react";
...
function Manage() {
  ...
  function InsightsHeader() {
    ...
    const computedProLabore = computedProLaboreMonthly(s, new Date());
    const pjBase = computedProLabore / 0.28;
    ...
    return (
      <Section title="Header de Insights" subtitle="Leitura rápida da saúde fiscal e operacional">
        <div className="grid gap-3 md:grid-cols-3">
          <InsightCard
            icon={ok ? <Rocket className="h-4 w-4 text-success" /> : <Shield className="h-4 w-4 text-primary" />}
            eyebrow="INSS"
            title={ok ? "Teto atingido" : "Abaixo do teto"}
            value={`${pct.toFixed(0)}%`}
            detail={`Fixos ${brl2(totalGross)} · Teto ${brl2(INSS_CEILING)}`}
          ...
          <InsightCard
            ...
            eyebrow="Pró-Labore"
            title="Automático"
            value={brl2(computedProLabore)}
            detail={`Base PJ Simples: ${brl2(pjBase)}`}
            tone="success"
          />

          <InsightCard
            icon={criticalDocs.length > 0 ? <ShieldAlert className="h-4 w-4 text-warning" /> : <ShieldCheck className="h-4 w-4 text-success" />}
            eyebrow="Cofre"
            title={criticalDocs.length > 0 ? "Documentos em atenção" : "Sem alertas críticos"}
            value={String(criticalDocs.length)}
            detail={criticalDocs.length > 0 ? "vencidos ou próximos de vencer" : "próximos 60 dias limpos"}
            tone={criticalDocs.length > 0 ? "warning" : "success"}
          >
            {criticalDocs.length > 0 && (
              <div className="mt-3 space-y-1.5">
                {criticalDocs.slice(0, 2).map((doc) => (
  ...
  function InsightCard({
    ...
    const valueColor = tone === "warning" ? "text-warning" : tone === "success" ? "text-success" : "text-foreground";
    return (
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.035] p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="h-8 w-8 rounded-xl bg-black/30 border border-white/[0.06] flex items-center justify-center">
            {icon}
          </div>
        </div>
        <p className={`font-display text-2xl mt-3 tabular-nums ${valueColor}`}>{value}</p>
  ...
  function OperationalAccordion() {
    return (
      <Section title="Configuração Operacional" subtitle="Cadastros e parâmetros agrupados por área de gestão">
        <Accordion.Root
          type="multiple"
          defaultValue={["revenue", "costs", "wealth", "documents"]}
          className="space-y-3"
        >
          <AccordionGroup
            value="revenue"
            title="Fontes de Receita"
            subtitle={`${s.workplaces.length} hospital(is) · ${s.fixedIncomes.length} vínculo(s) fixo(s)`}
          >
            <WorkplacesCard />
            <FixedIncomeCard />
          </AccordionGroup>
          ...
          <AccordionGroup
            value="costs"
            title="Custos & Passivos"
            subtitle={`${s.fixedCosts.length} custo(s) fixo(s) · ${s.debts.length} dívida(s)`}
          >
            <FixedCard />
            <DebtsCard />
          </AccordionGroup>
          ...
          <AccordionGroup
            value="wealth"
            title="Patrimônio & Logística"
            subtitle={`${s.vehicle.model.trim() ? s.vehicle.model : "sem veículo"} · ${s.goals.length} meta(s)`}
          >
            <VehicleCard />
            <GoalsCard />
          </AccordionGroup>
          ...
          <AccordionGroup
            ...
            subtitle={`${s.documents.length} documento(s) cadastrado(s)`}
          >
            <DocumentsCard />
          </AccordionGroup>
        </Accordion.Root>
  ...
  function AccordionGroup({
    ...
    return (
      <Accordion.Item value={value} className="rounded-2xl border border-white/[0.06] bg-white/[0.025] overflow-hidden">
        <Accordion.Header>
          <Accordion.Trigger className="group w-full px-4 py-4 flex items-center justify-between gap-4 text-left hover:bg-white/[0.025] transition">
        ...
        <Accordion.Content className="border-t border-white/[0.06] px-4 pb-4 data-[state=open]:animate-in data-[state=open]:fade-in data-[state=closed]:animate-out data-[state=closed]:fade-out">
          <div className="[&>section]:pt-5 [&>section]:pb-1 [&>section>div:first-child]:mb-3 [&>section+section]:mt-3">
            {children}
          </div>
        </Accordion.Content>
      </Accordion.Item>
          ...
          {s.workplaces.map((w) => (
            <div key={w.id} className="p-3 flex items-start justify-between gap-3">
              <div className="flex gap-3 items-start min-w-0 flex-1">
                <span
                  className="h-4 w-4 mt-1 rounded-full border border-white/20 shrink-0"
                  style={{ background: w.color }}
                  aria-label={`Cor do hospital ${w.name}`}
                />
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3">
                    <p className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">Cor no calendário</p>
                    <div className="flex flex-wrap gap-1.5">
                      {HOSPITAL_COLOR_PALETTE.map((color) => (
                        <button
                          key={color}
                          type="button"
                          onClick={() => s.updateWorkplace(w.id, { color })}
                          className={`h-6 w-6 rounded-lg border transition ${
                            w.color === color ? "border-white/80 ring-2 ring-white/15" : "border-white/10 hover:border-white/40"
                          }`}
                          style={{ background: color }}
                          aria-label={`Usar cor ${color} para ${w.name}`}
                        />
                      ))}
                    </div>
                  </div>
