...
export function LandingPage() {
  return (
    <main className="premium-shell min-h-screen text-foreground">
      <LandingHeader />
      <HeroSection />
      <SocialProof />
      <FeaturesSection />
    ...
    <header className="sticky top-0 z-50 border-b border-border/80 bg-background/86 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-6xl items-center justify-between px-5">
        <Link to="/" className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-primary-foreground shadow-[0_0_28px_rgba(29,112,255,0.22)]">
            <Command className="h-4 w-4" strokeWidth={2.5} />
          </div>
          <div>
            <p className="text-base font-bold tracking-tight text-foreground">DocFin</p>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Wealth Médico</p>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/dashboard"
            className="hidden rounded-lg border border-border px-4 py-2.5 text-sm font-semibold text-foreground transition hover:border-primary/45 hover:bg-primary/10 sm:inline-flex"
          >
            Entrar
          </Link>
          <Link
            to="/dashboard"
            className="rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-[0_0_28px_rgba(29,112,255,0.20)] transition hover:bg-primary/90"
          >
...
function HeroSection() {
  return (
    <section id="produto" className="overflow-hidden px-5 py-24 md:py-28 lg:py-32">
      <div className="mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[1fr_0.92fr]">
        <div>
          <p className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-surface-elevated px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground shadow-sm">
            <ShieldCheck className="h-4 w-4 text-[#0284C7]" />
            Construído para a rotina de plantões
          </p>

          <h1 className="max-w-4xl text-5xl font-black leading-[0.98] tracking-[-0.055em] text-foreground md:text-6xl lg:text-[4rem]">
          ...
          <div className="mt-12">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Rotina de confiança para quem atua em</p>
            <div className="mt-4 flex flex-wrap gap-3">
              {HOSPITALS.map((hospital) => (
                <div key={hospital} className="rounded-lg border border-border bg-surface-elevated px-4 py-2 text-sm font-semibold text-muted-foreground shadow-sm">
                  {hospital}
                </div>
              ))}
            </div>
...
function HeroMockup() {
  return (
    <div className="relative mx-auto w-full max-w-[430px]">
      ...
      <div className="relative mx-auto w-[min(100%,360px)] rounded-[2.2rem] border border-border bg-surface-elevated p-3 shadow-[0_34px_90px_rgba(1,7,22,0.42)]">
...
function SocialProof() {
  return (
    <section className="border-y border-border bg-surface/60 px-5 py-10">
...
function HowItWorksSection() {
  return (
    <section id="como-funciona" className="bg-surface/35 px-5 py-24 md:py-28">
...
function SecuritySection() {
  return (
    <section id="seguranca" className="bg-surface px-5 py-24 text-foreground md:py-28">
...
function FinalCta() {
  return (
    <section className="px-5 py-24">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl border border-border bg-surface-elevated p-8 text-foreground shadow-[0_28px_90px_rgba(1,7,22,0.32)] md:p-14">
        <div className="relative">
          ...
          <div className="relative max-w-3xl">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link to="/dashboard" className="inline-flex h-12 items-center justify-center rounded-lg bg-primary px-6 text-sm font-bold text-primary-foreground transition hover:bg-primary/90">
                Solicitar demonstração
              </Link>
      ...
      <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-[1.1fr_1fr_1fr_1fr]">
        ...
        {FOOTER_COLUMNS.map((column) => (
          <div key={column.title}>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">{column.title}</p>
...
function SectionHeader({ label, title, body }: { label: string; title: ReactNode; body: string }) {
  return (
    <div className="max-w-3xl">
      ...
      <h2 className="mt-4 text-4xl font-black tracking-[-0.04em] text-foreground md:text-5xl">{title}</h2>
...
function FeatureCard({ title, label, body, icon }: { title: string; label: string; body: string; icon: ReactNode }) {
  return (
    <div className="premium-card rounded-2xl p-8 transition hover:-translate-y-1 hover:border-primary/40">
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary">
        {icon}
      </div>
      <p className="mt-7 text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">{label}</p>
      <h3 className="mt-3 text-2xl font-bold tracking-[-0.025em] text-foreground">{title}</h3>
...
function StepItem({ number, title, body }: { number: string; title: string; body: string }) {
  return (
    <div className="flex gap-5 rounded-2xl bg-surface-elevated/70 p-5">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-primary/30 bg-primary/10 font-mono text-sm font-bold text-primary">{number}</div>
      <div>
...
function PlatformMockup() {
  return (
    <div className="rounded-3xl border border-border bg-surface p-4 shadow-[0_28px_80px_rgba(1,7,22,0.30)]">
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center justify-between border-b border-border pb-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Painel</p>
            <h3 className="mt-1 text-2xl font-bold text-foreground">Fechamento de maio</h3>
          </div>
...
function ProofMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="premium-card rounded-2xl p-5 text-center">
      ...
      <p className="mt-2 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{label}</p>
    </div>
  );
}
...
function MockMetric({ title, value, accent }: { title: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-2xl border border-border bg-surface-elevated p-4">
      ...
      <p className={`mt-2 font-mono text-xl font-bold ${accent ? "text-success" : "text-foreground"}`}>{value}</p>
    </div>
  );
}
...
function MockLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between border-b border-border py-3 last:border-0">
      ...
      <span className="rounded-full border border-border bg-card px-3 py-1 text-xs font-bold text-muted-foreground">{value}</span>
    </div>
  );
}
...
function PrimaryButton({ children }: { children: ReactNode }) {
  return (
    <Link
      to="/dashboard"
      className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-primary px-6 text-sm font-bold text-primary-foreground shadow-[0_0_28px_rgba(29,112,255,0.20)] transition hover:bg-primary/90"
    >
      {children}
...
function SecondaryButton({ href, children }: { href: string; children: ReactNode }) {
  return (
    <a
      href={href}
      className="inline-flex h-12 items-center justify-center rounded-lg border border-border px-6 text-sm font-bold text-foreground transition hover:border-primary/45 hover:bg-primary/10"
    >
      {children}
    </a>
  );
