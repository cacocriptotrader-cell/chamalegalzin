...
function FinancialSummaryWidget({ summary }: { summary: ReturnType<typeof getFinancialSummarySnapshot> }) {
  ...
  const taxesAndExpenses = summary.pjTax + summary.proLaboreTaxes + summary.pjCosts;
    ...
    >
      <div className="rounded-2xl border border-border bg-surface-elevated p-4">
        ...
        <div className="mt-5">
          ...
          <div className="h-3 rounded-full bg-primary/80 overflow-hidden border border-border">
            <div className="h-full bg-rose-300/70" style={{ width: `${expenseRatio}%` }} />
          </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: "linear-gradient(135deg, rgba(37,99,235,0.16), rgba(15,26,48,0.94) 55%, rgba(148,163,184,0.10))",
        }}
      >
    ...
    >
      <div className="rounded-2xl border border-border bg-surface-elevated/50 p-4">
        ...
        <button
          ...
          className="mt-4 w-full rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition"
        >
    ...
    >
      {shifts.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-6 text-center">
          ...
          <h3 className="font-display text-lg mt-3">Nenhuma pendência</h3>
        ...
        <div className="grid gap-3">
          {shifts.map((shift) => (
            <DraftShiftRow key={shift.id} shift={shift} active={selectedId === shift.id} onEdit={() => setSelectedId(shift.id)} />
          ))}
...
function DraftShiftRow({ shift, active, onEdit }: { shift: Shift; active: boolean; onEdit: () => void }) {
  const store = useStore();
  ...
  return (
    <button
      type="button"
      onClick={onEdit}
      className={`rounded-2xl border p-4 text-left transition ${active ? "border-primary/40 bg-primary/10" : "border-border bg-surface-elevated/50 hover:border-primary/30"}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{formatEventDate(shift.date)} · Draft</p>
          <h3 className="mt-1 truncate text-sm font-semibold">{workplace?.name ?? "Hospital não cadastrado"}</h3>
          <p className="mt-1 text-xs text-muted-foreground">{shift.hours}h · {shift.procedure || "Plantão rápido"}</p>
        </div>
        <span className="font-mono text-sm text-primary tabular-nums">{brl2(shift.gross)}</span>
      </div>
    </button>
  );
}
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-32px)] max-w-3xl -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-border bg-card p-0 shadow-[0_24px_80px_rgba(2,6,23,0.34)] max-h-[88vh] overflow-hidden">
          <div className="flex items-start justify-between gap-4 border-b border-border bg-surface-elevated/60 px-5 py-4">
            <div>
...
function ConsolidateShiftForm({ shift, onClose }: { shift: Shift; onClose: () => void }) {
  ...
  const [settlementAdjustment, setSettlementAdjustment] = useState(shift.settlementAdjustment ?? 0);
  const [taxRegime, setTaxRegime] = useState<TaxRegime | "">(shift.taxRegimeOverride ?? "");
  ...
  function consolidate() {
    ...
    store.updateShift(shift.id, {
      ...
      privateTransportCost: transportMode === "PRIVATE_TRANSPORT" ? privateTransportCost : 0,
      extraCost: transportMode === "PERSONAL_VEHICLE" ? extraCost : 0,
      settlementAdjustment: Math.max(0, settlementAdjustment),
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 rounded-2xl border border-border bg-surface-elevated/50 p-4 sm:grid-cols-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-1.5">
          ...
          <input type="date" value={date} onChange={(event) => setDate(event.target.value)} className={dashboardInputCls} />
        </label>
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Hospital</span>
          <select value={workplaceId} onChange={(event) => setWorkplaceId(event.target.value)} className={dashboardInputCls}>
      ...
      <div className="mt-3 rounded-2xl border border-border bg-surface-elevated/50 p-3">
      ...
      <div className="grid gap-3 rounded-2xl border border-border bg-surface-elevated/50 p-3 md:grid-cols-[1fr_1fr]">
        <label className="space-y-1.5">
          <span className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Ajustes / retenções / repasses (R$)</span>
          <input
            type="number"
            min={0}
            value={settlementAdjustment || ""}
            onChange={(event) => setSettlementAdjustment(Math.max(0, +event.target.value || 0))}
            className={`${dashboardInputCls} text-right tabular-nums`}
          />
        </label>
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
        <button type="button" onClick={discard} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold text-muted-foreground hover:text-destructive transition">
        ...
        <button
          ...
          onClick={consolidate}
          disabled={!isValid}
          className="rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground"
        >
          Consolidar registro
        </button>
      </div>
...
function getFinancialSummarySnapshot(store: StoreState, now: Date) {
  ...
  for (const shift of store.shifts) {
    ...
    addRevenueByRegime(shift.gross, getShiftRegime(store, shift));
  }
...
const dashboardInputCls = "w-full rounded-xl border border-border bg-card px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40";
