    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <div className="border-b border-border bg-background/92 backdrop-blur">
        <header className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5">
          ...
          <Link
            to="/dashboard"
            className="inline-flex h-10 items-center gap-2 rounded-md border border-border bg-card px-4 text-xs font-semibold text-foreground transition duration-200 hover:border-primary/50 hover:text-primary"
          >
      ...
      <section className="relative px-5 py-20 md:py-28">
        <div className="absolute inset-0 opacity-[0.05] [background-image:linear-gradient(rgba(148,163,184,0.42)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.42)_1px,transparent_1px)] [background-size:72px_72px]" />
        <div className="relative mx-auto grid max-w-7xl gap-16 lg:grid-cols-[0.94fr_1.06fr] lg:items-center">
          <div>
            ...
            <div className="mt-10 flex flex-col gap-3 sm:flex-row">
              <Link
                to="/dashboard"
                className="group inline-flex h-12 items-center justify-center gap-2 rounded-md border border-primary/50 bg-primary px-6 text-sm font-semibold text-primary-foreground transition duration-200 hover:border-blue-300/60 hover:shadow-[0_0_0_1px_rgba(37,99,235,0.28)]"
              >
                Iniciar Planeamento
              ...
              <a
                href="#fechamento"
                className="inline-flex h-12 items-center justify-center rounded-md border border-border bg-card px-6 text-sm font-semibold text-muted-foreground transition duration-200 hover:border-primary/50 hover:text-foreground"
              >
                Ver fluxo contábil
              </a>
            </div>
      ...
      <section className="px-5 py-28 md:py-36">
        <div className="mx-auto max-w-7xl border-y border-border py-16 md:grid md:grid-cols-[1fr_auto] md:items-center md:gap-12">
          ...
          <Link
            to="/dashboard"
            className="mt-10 inline-flex h-12 items-center justify-center gap-2 rounded-md border border-primary/50 bg-primary px-6 text-sm font-semibold text-primary-foreground transition duration-200 hover:border-blue-300/60 md:mt-0"
          >
            Iniciar Planeamento
...
function HeroCockpitMockup() {
  return (
    <div className="relative">
      <div className="border border-border bg-card p-3 shadow-[0_24px_80px_rgba(2,6,23,0.28)]">
      ...
      <div className="absolute -bottom-5 -right-4 hidden border border-primary/35 bg-primary/10 px-4 py-3 text-xs text-blue-200 md:block">
...
function QuickCaptureChaosMockup() {
  ...
  return (
    <div className="grid gap-5 md:grid-cols-[0.92fr_1.08fr] md:items-center">
      <div className="border border-border bg-surface p-4 shadow-[0_18px_56px_rgba(2,6,23,0.25)]">
...
function CalendarTimelineMockup() {
  ...
  return (
    <div className="border border-border bg-card p-4 shadow-[0_22px_70px_rgba(2,6,23,0.24)]">
      <div className="border border-border bg-surface p-5">
        ...
        <div className="grid gap-3 md:grid-cols-5">
          {days.map((item) => (
            <div
              key={item.day}
              className={`min-h-32 border p-3 transition ${item.active ? "border-primary/45 bg-primary/10" : "border-border bg-card"} ${item.ghost ? "opacity-45" : ""}`}
            >
              ...
              {item.active && <span className="mt-4 inline-block border border-primary/45 px-2 py-1 text-[10px] text-blue-200">triagem</span>}
            </div>
          ))}
        </div>
      </div>
...
function InboxAccountingMockup() {
  return (
    <div className="relative border border-border bg-surface p-4 shadow-[0_22px_70px_rgba(2,6,23,0.24)]">
      <div className="border border-border bg-card p-5">
        <div className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-2">
...
function AuditRow({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "neutral" | "warning" }) {
  return (
    <div className="flex items-center justify-between gap-4 border border-border bg-surface-elevated px-3 py-3 text-xs">
      <span className="text-muted-foreground">{label}</span>
      <span className={`font-mono ${tone === "warning" ? "text-warning" : "text-foreground"}`}>{value}</span>
    </div>
  );
}
