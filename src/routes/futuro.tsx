  ...
  const renderScenario = () => {
    switch (pgblAdvantage.scenario) {
      ...
      case "B":
        return (
          <p className="text-zinc-200 text-sm leading-relaxed">
            Insight: Atenção Fiscal. O seu faturamento PF atingiu a zona de transição para a Declaração Completa. Um aporte de até <strong className="text-success">{brl2(pgblAdvantage.idealLimit)}</strong> em PGBL pode otimizar a sua restituição. Analise com o seu contador.
          </p>
        );
      case "C":
        return (
          <p className="text-zinc-200 text-sm leading-relaxed">
            Insight: Oportunidade Fiscal. O seu volume de faturamento PF exige a Declaração Completa. Aportar o teto de <strong className="text-success">{brl2(pgblAdvantage.idealLimit)}</strong> em PGBL este ano garante uma economia direta de <strong className="text-success">{brl2(pgblAdvantage.taxSavings)}</strong> em impostos. Para valores acima desse teto, utilize CDB.
          </p>
        );
    }
  };
...
function SmartAllocator() {
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto rounded-3xl bg-card border border-border p-6 md:p-8 shadow-[0_30px_80px_-30px_rgba(2,6,23,0.62)]">
        {/* Header */}
        <div className="text-center">
          ...
          <div className="mt-2 inline-flex items-baseline gap-2">
            ...
            <input
              ...
              className="bg-transparent outline-none border-none text-success font-light tabular-nums tracking-tight text-5xl md:text-6xl text-center w-[7ch] focus:w-[9ch] transition-all"
            />
          </div>
        ...
        <div className="mt-8 space-y-2">
          {values[1] > 0 && (
            <Verdict>
              A alocação de <strong className="text-success tabular-nums">{brl(values[1])}</strong>/mês
              economiza aproximadamente{" "}
              <strong className="text-success tabular-nums">{brl(interestSaved)}</strong> em juros futuros (5 anos · {debtRate.toFixed(1)}% a.a.).
            </Verdict>
          )}
          {values[0] > 0 && pgblEligible && (
            <Verdict>
              Alocação otimizada para redução na base de cálculo do IRPF — economia anual estimada de{" "}
              <strong className="text-success tabular-nums">{brl(irSaved)}</strong>.
            </Verdict>
          )}
          {values[2] > 0 && (
            <Verdict>
              <strong className="text-success tabular-nums">{brl(values[2] * 12)}</strong>/ano direcionados
...
function Bucket({
  ...
  return (
    <div
      className={`rounded-2xl bg-background/60 border border-border p-4 flex flex-col items-center text-center ${disabled ? "opacity-50" : ""}`}
    ...
    >
      ...
      <p className="text-[11px] text-muted-foreground tabular-nums">{(pct * 100).toFixed(0)}%</p>
      <input
        type="range"
        min={0}
        max={pctMax}
      ...
      {disabled && disabledHint && (
        <p className="mt-2 text-[10px] text-muted-foreground leading-snug">{disabledHint}</p>
      )}
    </div>
  );
}
...
function Verdict({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2 rounded-xl border border-success/20 bg-success/[0.04] px-3 py-2.5">
  ...
  return (
    <Section title="Independência Financeira" subtitle="Simulador patrimonial interativo — arraste e veja a bola de neve crescer">
      <TooltipProvider delayDuration={150}>
        <div className="rounded-2xl bg-card/50 backdrop-blur-md border border-border p-5 md:p-6 space-y-6">
          {/* HERO RESULT */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <div>
              ...
              <p className="font-display text-5xl md:text-6xl font-light tabular-nums tracking-tight text-success mt-1 drop-shadow-[0_0_30px_rgba(34,197,94,0.18)]">
                {brl(monthly)}
              </p>
              <p className="text-xs text-zinc-400 mt-1">
                por <span className="text-success">{years} anos</span> · alvo {brl(targetWealth)}
              </p>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">O Efeito Bola de Neve</p>
              <div className="flex items-center gap-3 text-[10px]">
                <span className="flex items-center gap-1.5 text-zinc-400">
                  <span className="h-2 w-2 rounded-full bg-zinc-500" /> Aportes
                </span>
                <span className="flex items-center gap-1.5 text-success">
                  <span className="h-2 w-2 rounded-full bg-success shadow-[0_0_8px_rgba(34,197,94,0.35)]" /> Juros compostos
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <InsightBox>
              <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Aporte total em {years} anos</p>
              <p className="text-2xl font-light text-zinc-100 mt-1 tabular-nums">{brl(totalContrib)}</p>
            </InsightBox>
            <InsightBox>
              <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Ganho com juros compostos</p>
              <p className="text-2xl font-light text-success mt-1 tabular-nums">{brl(totalInterest)}</p>
            </InsightBox>
          </div>
        </div>
      </TooltipProvider>
      ...
      <div className="rounded-2xl bg-card/50 backdrop-blur-md border border-border p-5 md:p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-xl bg-background/60 border border-border p-4">
            <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">IR sem PGBL (ano)</p>
            <p className="text-3xl font-light text-zinc-100 mt-2 tabular-nums">{brl(irSemPgbl)}</p>
          </div>
          <div className="rounded-xl bg-background/60 border border-border p-4">
            <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">IR com PGBL (ano)</p>
            <p className="text-3xl font-light text-success mt-2 tabular-nums">{brl(irComPgbl)}</p>
          </div>
          <div className="rounded-xl bg-background/60 border border-border p-4">
            <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Economia anual</p>
            <p className="text-3xl font-light text-success mt-2 tabular-nums">{brl(taxSaving)}</p>
            <p className="text-[10px] text-muted-foreground mt-1">({pctSaved.toFixed(1)}% de redução)</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-xl bg-gradient-to-br from-emerald-900/20 to-transparent border border-success/20 p-4">
            <p className="text-[10px] uppercase tracking-[0.15em] text-success">Reinvestimento de 30 anos</p>
            <p className="text-2xl font-light text-success mt-2 tabular-nums">{brl(docfinWealth)}</p>
            <p className="text-xs text-zinc-400 mt-1">
          ...
          <div className="rounded-xl bg-background/60 border border-border p-4">
            <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Limite de Dedução Simplificada</p>
...
function MiniStat({ label, value, tone }: { label: string; value: string; tone: "neutral" | "emerald" }) {
  const color = tone === "emerald" ? "text-success" : "text-zinc-400";
  return (
      ...
      <div className="grid grid-cols-3 gap-2">
        {conversions.map((c) => (
          <div key={c.label} className="text-center">
            <p className="text-lg font-light text-success tabular-nums">{nf0.format(c.value)}</p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm font-light text-success tabular-nums">
