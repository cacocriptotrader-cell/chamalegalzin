...
import {
  ...
  SIMPLIFIED_DEDUCTION_CAP, calculateExpectedPaymentDate, isConsolidatedRecord,
} from "@/lib/store";
...
export function SmartActionFeed() {
  ...
  const insights = useMemo<InsightItem[]>(() => {
    ...
    const monthShifts = store.shifts.filter((sh) => {
      if (!isConsolidatedRecord(sh)) return false;
      ...
      out.push({
        ...
        body: (
          <>
            ...
            adicional de <span className="text-success tabular-nums">{brl(pgbl.taxSavings)}</span> em IR.
          </>
        ),
      });
    }
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            {insights.map((i) => (
              <div
                key={i.id}
                className="rounded-xl bg-slate-900/80 border border-border px-4 py-3.5 hover:border-border transition-colors"
              >
          ...
          <div className="space-y-2">
            {actions.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className={`group relative rounded-xl bg-card border border-border pl-4 pr-3 py-3 flex items-start gap-3 transition-all hover:border-primary/35
