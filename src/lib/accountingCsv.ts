...
function buildAccountingRows(store: StoreState, month: number, year: number, competence: string): AccountingCsvRow[] {
  ...
  for (const shift of store.shifts) {
    ...
    if (!shift.invoiceIssueDate || !inMonth(shift.invoiceIssueDate)) continue;
    ...
    rows.push({
      ...
      invoiceIssueDate: shift.invoiceIssueDate,
  ...
  for (const surgery of store.surgeries) {
    ...
    if (!surgery.invoiceIssueDate || !surgery.invoiceNumber || !inMonth(surgery.invoiceIssueDate)) continue;
    ...
    if (surgery.myRole === "TITULAR") {
      ...
      rows.push({
        ...
        invoiceIssueDate: surgery.invoiceIssueDate,
        invoiceNumber: surgery.invoiceNumber,
  ...
  return rows.sort((a, b) => {
    if (a.invoiceIssueDate !== b.invoiceIssueDate) return a.invoiceIssueDate.localeCompare(b.invoiceIssueDate);
