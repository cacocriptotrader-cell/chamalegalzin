...
function loadState(): StoreState {
  ...
  try {
    ...
    merged.shifts = (merged.shifts || []).map((s: any) => asPJ(s));
    ...
    merged.surgeries = (merged.surgeries || []).map((s: any) => asPJ(s));
...
export function computeShift(s: StoreState, shift: Shift): ShiftMath {
  ...
  if (shift.transportMode === "PRIVATE_TRANSPORT") {
    ...
    return { km: 0, fuelCost: 0, wearCost: 0, tax, net, logistics };
  }
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const actions: StoreActions = {
    ...
    addShift: (s) => setState((p) => {
      ...
      return { ...p, shifts: [...p.shifts, { id: uid(), ...s, expectedPaymentDate, projectedPaymentDate: expectedPaymentDate, entityDomain: "PJ" }] };
    }),
    ...
    addSurgery: (s) => setState((p) => ({ ...p, surgeries: [...p.surgeries, { id: uid(), ...s, entityDomain: "PJ" } as SurgeryRecord] })),
