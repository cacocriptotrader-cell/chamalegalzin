...
function loadState(): StoreState {
  ...
  try {
    ...
    merged.shifts = (merged.shifts || []).map((s: any) => ({ ...asPJ(s), recordStatus: s.recordStatus ?? "consolidated" }));
    ...
    merged.surgeries = (merged.surgeries || []).map((s: any) => ({ ...asPJ(s), recordStatus: s.recordStatus ?? "consolidated" }));
...
export function computeShift(s: StoreState, shift: Shift): ShiftMath {
  ...
  const settlementAdjustment = Math.max(0, shift.settlementAdjustment || 0);
  ...
  if (shift.transportMode === "PRIVATE_TRANSPORT") {
    ...
    const net = shift.gross - tax - logistics - settlementAdjustment;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const actions: StoreActions = {
    ...
    addSurgery: (s) => setState((p) => ({ ...p, surgeries: [...p.surgeries, { id: uid(), recordStatus: "consolidated", ...s, entityDomain: "PJ" } as SurgeryRecord] })),
