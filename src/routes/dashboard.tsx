...
function Dashboard() {
  ...
  return (
    <div className="space-y-5 px-0 py-5 animate-fade-in md:py-7">
      <header className="premium-card relative overflow-hidden rounded-3xl p-5 md:p-7">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-4 font-display text-3xl text-foreground md:text-4xl">Hoje, {firstName}</h1>
          ...
          <div className="premium-panel rounded-2xl px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
            <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Mês atual</p>
            <p className="font-mono text-sm font-semibold capitalize tabular-nums text-foreground">
      ...
      {isZeroData ? (
        ...
        <div className="space-y-4">
          ...
          <ProgressiveSection
            ...
            title="Operação, caixa e patrimônio"
            defaultOpen={draftShifts.length === 0}
          >
    ...
    <section className="premium-card rounded-2xl p-4 md:p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <button
          type="button"
          onClick={onOpen}
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-primary/35 bg-primary/10 px-4 py-2.5 text-sm font-semibold text-primary transition hover:bg-primary/15"
        >
...
function ExecutiveKpiDeck({
  ...
  const taxStatus = !summary.hasSimpleRevenue
    ? "Sem base"
    : summary.factorSafe
      ? "Meta segura"
      : "Atenção";
  const taxTone = !summary.hasSimpleRevenue ? "neutral" : summary.factorSafe ? "success" : "warning";
  ...
  return (
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      />
      <ExecutiveKpiCard
        label="Índice tributário"
        value={summary.hasSimpleRevenue ? `${summary.factorPercent.toFixed(1)}%` : taxStatus}
        detail={summary.hasSimpleRevenue ? "Meta gerencial: 28%" : "Sem PJ Simples no período"}
...
function ExecutiveKpiCard({
  ...
  return (
    <article className="premium-card rounded-2xl p-4 shadow-[0_18px_64px_rgba(0,0,0,0.22)] transition hover:border-primary/35">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">{label}</p>
          <p className={`mt-3 font-mono text-2xl font-semibold tabular-nums tracking-tight ${toneClass}`}>
            {value}
          </p>
        </div>
...
function ZeroDataLaunchPad({ onQuickCapture }: { onQuickCapture: () => void }) {
  return (
    <section className="premium-card overflow-hidden rounded-3xl p-5 md:p-7">
        ...
        <div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-semibold text-primary-foreground shadow-[0_0_28px_rgba(15,118,110,0.24)] transition hover:bg-primary/90"
            >
            ...
            <Link
              to="/lancar-plantoes"
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-border px-5 text-sm font-semibold text-muted-foreground transition hover:border-primary/50 hover:text-primary"
            >
              Abrir mesa em lote
        ...
        <div className="rounded-2xl border border-border bg-black/30 p-4">
          <div className="space-y-3">
            {[
              ...
              <div key={label} className="flex items-center justify-between gap-4 rounded-xl border border-border/80 bg-surface/70 px-4 py-3">
...
function ProgressiveSection({
  ...
  return (
    <details className="group premium-card rounded-2xl p-4 md:p-5" open={defaultOpen}>
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
        ...
        <span className="rounded-full border border-border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground transition group-open:border-primary/35 group-open:text-primary">
          Ver detalhes
        </span>
      </summary>
          ...
          {events.map((event) => (
            <div key={event.id} className="premium-panel rounded-2xl p-4">
              <div className="flex items-start justify-between gap-4">
                ...
                <p className="shrink-0 font-mono text-xl font-semibold tabular-nums text-success">{brl(event.value)}</p>
              </div>
            </div>
          ))}
        </div>
...
function FinancialSummaryWidget({ summary }: { summary: ReturnType<typeof getFinancialSummarySnapshot> }) {
  ...
  const estimatedTaxes = summary.pjTax + summary.proLaboreTaxes + summary.pfTax;
  const realExpenses = summary.pjCosts + summary.pjDeductions + summary.pfDeductions;
  const totalOutflows = estimatedTaxes + realExpenses;
  const expenseRatio = summary.pjRevenue > 0 ? Math.min(100, (totalOutflows / summary.pjRevenue) * 100) : 0;
  ...
  return (
    ...
    >
      <div className="premium-panel rounded-2xl p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className={`mt-2 font-mono text-4xl font-semibold tabular-nums tracking-tight ${resultTone}`}>
              {brl(summary.consolidatedNet)}
            </p>
            <p className="text-xs text-muted-foreground mt-2">Resultado líquido depois de impostos estimados e despesas reais do mês.</p>
          </div>
      ...
      <div className="grid gap-3 mt-3">
        ...
        <MiniMetric label="Impostos Estimados" value={brl2(estimatedTaxes)} tone="danger" />
        {realExpenses > 0 && <MiniMetric label="Despesas Reais" value={brl2(realExpenses)} tone="danger" />}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className={`mt-2 font-mono text-4xl font-semibold tabular-nums tracking-tight ${wealth.netWorth >= 0 ? "text-success" : "text-destructive"}`}>
          {brl(wealth.netWorth)}
        </p>
...
function DraftInbox({ shifts }: { shifts: Shift[] }) {
  ...
  const [batchValidationPending, setBatchValidationPending] = useState(false);
  ...
  useEffect(() => {
    if (!batchValidationPending) return;

    const selectedDrafts = shifts.filter((shift) => selectedIds.includes(shift.id));
    if (selectedDrafts.length === 0) {
      setBatchValidationPending(false);
      return;
    }

    const missing = selectedDrafts.find((shift) => {
      const workplace = store.workplaces.find((item) => item.id === shift.workplaceId);
      return !shift.taxRegimeOverride && !workplace?.regime;
    });

    if (missing) {
      const workplace = store.workplaces.find((item) => item.id === missing.workplaceId);
      setBatchValidationError(
        `Defina a regra de faturamento para o Hospital ${workplace?.name ?? "não cadastrado"} antes de consolidar este lote.`,
      );
    } else {
    ...
    setBatchValidationPending(false);
  }, [batchValidationPending, selectedIds, shifts, store.workplaces]);
  ...
  async function openBatchConsolidation() {
    if (selectedIds.length === 0 || batchRefreshing) return;

    setBatchValidationError(null);
    setBatchRefreshing(true);
    const status = await store.refreshOperationalState();
    setBatchRefreshing(false);

    if (status === "local") {
      toast.warning("Não consegui atualizar o banco agora. Vou usar os dados locais mais recentes.");
    }

    setBatchValidationPending(true);
  }
        ...
        <div className="grid gap-3">
          <div className="premium-panel flex flex-col gap-3 rounded-2xl p-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {selectedIds.length > 0 && (
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <button
                  type="button"
                  onClick={openBatchConsolidation}
                  disabled={batchRefreshing}
                  className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground transition hover:bg-primary/90 disabled:cursor-wait disabled:opacity-60"
                >
                  {batchRefreshing ? "Atualizando..." : `Consolidar ${selectedIds.length} Selecionado${selectedIds.length === 1 ? "" : "s"}`}
                </button>
              </div>
            )}
          </div>
          {batchValidationError && (
            <div className="rounded-2xl border border-warning/40 bg-warning/10 p-3 text-xs text-warning">
              <p>{batchValidationError}</p>
              <Link to="/empresa" className="mt-2 inline-flex font-semibold text-foreground underline underline-offset-4">
                Ajustar hospital
              </Link>
            </div>
          )}
...
function DraftShiftRow({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 text-left transition ${active ? "border-primary/45 bg-primary/10" : "premium-panel hover:border-primary/30"} ${syncing ? "opacity-60" : ""}`}
    >
      <div className="flex items-start justify-between gap-3">
...
function ConsolidateShiftForm({ shift, onClose }: { shift: Shift; onClose: () => void }) {
  ...
  function discard() {
    ...
    setOptimisticAction("discarding");
    store.removeShift(shift.id);
    toast.success("Pendência descartada. Sincronizando em segundo plano.");
    onClose();
  }
    ...
    >
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
        <button
          type="button"
          onClick={discard}
          disabled={Boolean(optimisticAction)}
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold text-muted-foreground transition hover:border-destructive/40 hover:text-destructive disabled:cursor-not-allowed disabled:opacity-60"
        >
          <Trash2 className="h-4 w-4" />
          {optimisticAction === "discarding" ? "Descartando..." : "Descartar pendência"}
        </button>
        <button
          type="button"
          onClick={consolidate}
