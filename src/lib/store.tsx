...
export interface StoreActions {
  ...
  refreshOperationalState: () => Promise<PersistenceStatus>;
...
export function computeShift(s: StoreState, shift: Shift): ShiftMath {
  ...
  const regime = getShiftRegime(s, shift);
  if (regime === "SCP") {
    const net = calculateNetValue(shift.gross, shift.deductions, shift.settlementAdjustment);
    return { km: 0, fuelCost: 0, wearCost: 0, tax: 0, net, logistics: 0, settlementAdjustment };
  }
  const tax = computeTaxForRegime(shift.gross, regime, s);
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const actions: StoreActions = {
    ...
    refreshOperationalState: async () => {
      try {
        const remoteState = await fetchOperationalStateFromSupabase();
        ...
        logger.warn("Não foi possível atualizar dados operacionais. Mantendo cache local.", error);
        return "local";
      }
    },
    addWorkplace: (w) => {
    ...
    batchConsolidateShifts: (ids, consolidationData) => {
      ...
      const resolveTaxRegime = (shift: Shift, workplaces: Workplace[]): TaxRegime =>
        (consolidationData.taxationType as TaxRegime | undefined)
        || shift.taxRegimeOverride
        || workplaces.find((workplace) => workplace.id === shift.workplaceId)?.regime
        || "PJ_SIMPLES";
      const nextSyncedShifts = state.shifts
        ...
        .map((shift) => {
          const next: Shift = {
            ...shift,
            recordStatus: "consolidated",
            taxRegimeOverride: resolveTaxRegime(shift, state.workplaces),
        ...
        return {
          ...
          shifts: p.shifts.map((shift) => {
            ...
            const next: Shift = {
              ...shift,
              recordStatus: "consolidated",
              taxRegimeOverride: resolveTaxRegime(shift, p.workplaces),
              paymentStatus: consolidationData.paymentStatus as PaymentStatus,
              actualPaymentDate: consolidationData.paymentStatus === "PAID" ? (consolidationData.actualPaymentDate ?? fmtISO(new Date())) : undefined,
            };
            if (consolidationData.expectedPaymentDate) {
