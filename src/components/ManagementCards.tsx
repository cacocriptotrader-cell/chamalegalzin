...
import {
  brl2,
  calculateCLTNetMonthly,
  calculateINSSProgressive,
  calculateIRRF2026,
  computedProLaboreMonthly,
  daysUntil,
  DOC_KIND_TO_FIXED_COST,
  DOCUMENT_KIND_LABELS,
  inssCeilingReached,
  INSS_CEILING,
  monthlyFixedIncomeGrossTotal,
...
export function InssMonitorCard() {
  const s = useStore();
...
export function ProLaboreAutoCard() {
  const s = useStore();
  const now = new Date();
...
export function DocumentsCriticalAlerts() {
  const s = useStore();
  const criticalDocs = s.documents
    .map((d) => ({ ...d, days: daysUntil(d.expiresAt) }))
    .filter((d) => d.days <= 60)
    .sort((a, b) => a.days - b.days);

  return (
    <Section title="Alertas Críticos" subtitle="Documentos vencidos, vencendo hoje ou próximos de vencer">
      <div className="grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card/80 p-4 shadow-sm md:col-span-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Cofre</p>
              <h3 className="mt-1 text-sm font-medium text-foreground">
                {criticalDocs.length > 0 ? "Documentos em atenção" : "Sem alertas críticos"}
              </h3>
            </div>
            <div className="h-8 w-8 rounded-xl bg-surface border border-border flex items-center justify-center">
              {criticalDocs.length > 0 ? <ShieldAlert className="h-4 w-4 text-warning" /> : <ShieldCheck className="h-4 w-4 text-success" />}
            </div>
          </div>
          <p className={`font-display text-2xl mt-3 tabular-nums ${criticalDocs.length > 0 ? "text-warning" : "text-success"}`}>
            {criticalDocs.length}
          </p>
          <p className="text-[11px] text-muted-foreground mt-1">
            {criticalDocs.length > 0 ? "vencidos ou próximos de vencer" : "próximos 60 dias limpos"}
          </p>
          {criticalDocs.length > 0 && (
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              {criticalDocs.slice(0, 4).map((doc) => (
                <div key={doc.id} className="flex items-center justify-between gap-2 rounded-xl border border-border bg-surface/70 px-3 py-2 text-[11px]">
                  <span className="truncate text-foreground/80">{doc.label}</span>
                  <span className={doc.days <= 15 ? "text-destructive shrink-0" : "text-warning shrink-0"}>
                    {doc.days < 0 ? `${Math.abs(doc.days)}d vencido` : doc.days === 0 ? "hoje" : `${doc.days}d`}
                  </span>
                </div>
              ))}
            </div>
...
export function WorkplacesCard() {
  ...
  const [paymentOffsetMonths, setPaymentOffsetMonths] = useState(1);
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        ...
        if (err.name !== "AbortError") {
          logger.error("Falha ao buscar endereço do hospital.", err);
          setApiError(true);
          setSuggestions([]);
        }
      } finally {
  ...
  function add() {
    ...
    if (!Number.isFinite(paymentDay) || paymentDay < 1 || paymentDay > 31) return;
    if (!Number.isFinite(paymentOffsetMonths) || paymentOffsetMonths < 0) return;
    ...
    s.addWorkplace({
      ...
      cutOffDay,
      paymentDay,
      paymentTermDays: paymentOffsetMonths * 30,
    });
    setName("");
    setAddress("");
    setLat(null);
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-secondary/20 text-slate-400 inline-flex items-center gap-1">
                    <CalendarClock className="h-2.5 w-2.5" /> Corte dia {w.cutOffDay} → pagamento dia {w.paymentDay || 5} (+{cycleOffsetMonths(w.paymentTermDays)} ciclo)
                  </span>
                </div>
                <div className="grid gap-2 mt-3 sm:grid-cols-3">
                  ...
                  <label className="block">
                    ...
                    <input type="number" min={1} max={31} value={w.paymentDay || 5}
                      onChange={(e) => s.updateWorkplace(w.id, { paymentDay: Math.max(1, Math.min(31, +e.target.value || 5)) })}
                      className={inp + " text-[11px] tabular-nums"} />
                  </label>
                  <label className="block">
                    <span className="text-[9px] uppercase tracking-wider text-muted-foreground">Ciclo (+mês)</span>
                    <input type="number" min={0} max={6} value={cycleOffsetMonths(w.paymentTermDays)}
                      onChange={(e) => s.updateWorkplace(w.id, { paymentTermDays: Math.max(0, Math.min(6, +e.target.value || 0)) * 30 })}
                      className={inp + " text-[11px] tabular-nums"} />
                  </label>
                </div>
              </div>
      ...
      <div className="glass-card rounded-2xl p-4 space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <Field label="Dia de Pagamento *">
            <input type="number" min={1} max={31} required value={paymentDay}
              onChange={(e) => setPaymentDay(Math.max(1, Math.min(31, +e.target.value || 5)))}
              className={inp} placeholder="Ex: 20" />
          </Field>
          <Field label="Ciclos após corte">
            <input type="number" min={0} max={6} value={paymentOffsetMonths}
              onChange={(e) => setPaymentOffsetMonths(Math.max(0, Math.min(6, +e.target.value || 0)))}
              className={inp} placeholder="Ex: 1" />
          </Field>
        </div>
        <p className="text-sm text-slate-400 mt-1">
          Plantões até o dia <strong>{cutOffDay}</strong> entram no ciclo corrente e pagam no dia <strong>{paymentDay}</strong> em <strong>+{paymentOffsetMonths}</strong> ciclo. Depois do corte, o pagamento pula para o ciclo seguinte.
        </p>
...
function cycleOffsetMonths(paymentTermDays?: number) {
  return Math.max(0, Math.ceil(Math.max(0, paymentTermDays ?? 30) / 30));
}
...
export function FixedIncomeCard() {
  ...
  return (
    <Section title="Vínculos Fixos (CLT / Concurso)" subtitle={`Líquido mensal recorrente: ${brl2(monthlyFixedIncomeNet(s))}`}>
            ...
            <div className="flex items-start gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex flex-wrap gap-3 mt-1 text-[11px] text-slate-400">
                  ...
                  <span>Líquido calculado: <span className="font-mono text-success">{brl2(calculateCLTNetMonthly(i.grossMonthly))}</span></span>
                </div>
              </div>
            </div>
      ...
      <div className="rounded-2xl p-4 space-y-3 bg-surface/80 backdrop-blur border border-border">
        ...
        <div className="grid grid-cols-3 gap-2 rounded-lg border border-border bg-surface px-3 py-2.5">
          ...
          <span className="text-[11px] uppercase tracking-wider text-slate-400">Líquido: <strong className="font-mono text-success">{brl2(estimatedNet)}</strong></span>
        </div>
    ...
    <Section
      ...
      action={
        <button
          ...
          disabled={importing}
          className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border bg-secondary/60 text-[11px] font-medium text-muted-foreground hover:text-foreground hover:bg-secondary/70 transition disabled:opacity-50"
        >
          {importing ? (
...
export function GoalsCard() {
  ...
  return (
    <Section title="Metas Financeiras" subtitle={`${s.goals.length} meta${s.goals.length !== 1 ? "s" : ""} ativa${s.goals.length !== 1 ? "s" : ""}`}>
      ...
      <div className="glass-card rounded-2xl p-4 space-y-3 bg-surface/80 backdrop-blur border border-border">
...
export function VehicleCard() {
  ...
  return (
    <Section title="Veículo" subtitle="Consumo, depreciação e manutenção — base para logística real">
      {!hasVehicle ? (
        <div className="glass-card rounded-2xl p-12 flex flex-col items-center justify-center border-dashed border-border">
          <Car className="h-8 w-8 text-muted-foreground/40 mb-4" />
          <button
            onClick={() => s.setVehicle({ ...s.vehicle, model: "Novo Veículo" })}
            className="inline-flex items-center gap-2 rounded-xl bg-primary text-primary-foreground px-6 py-3 text-sm font-semibold hover:bg-primary/90 transition"
          >
