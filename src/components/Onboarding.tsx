...
export function Onboarding() {
  ...
  async function finishOnboarding() {
    ...
    try {
      ...
      const normalizedSpecialty = customSpecialty ? (specialtyName.trim() || "Outra") : specialtyName.trim();
        ...
        <div className="premium-card rounded-3xl p-5 md:p-7">
          ...
          <div className="space-y-5">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (value === "accountant") {
                          ...
                          setOnboardingGoal("ECONOMIZAR_TEMPO");
                        }
                      }}
            ...
            {isAccountant ? (
              ...
              <fieldset className="space-y-3">
                <legend className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                  <Stethoscope className="h-3.5 w-3.5" />
                  Especialidade médica <span className="normal-case tracking-normal text-muted-foreground/70">(opcional)</span>
                </legend>
                <div className="flex flex-wrap gap-2">
                  {specialtyOptions.map((option) => {
                    const active = customSpecialty ? option === "Outra" : specialtyName === option;
                    return (
                      <button
                        key={option}
                        type="button"
                        aria-pressed={active}
                        onClick={() => {
                          const isCustom = option === "Outra";
                          setCustomSpecialty(isCustom);
                          setSpecialtyName(isCustom ? "" : option);
                        }}
                        className={`min-h-11 rounded-full border px-4 text-sm font-medium transition ${
                          active
                            ? "border-primary/60 bg-primary/15 text-primary shadow-[0_0_22px_rgba(15,118,110,0.14)]"
                ...
                {customSpecialty && (
                  <input
                    value={specialtyName}
                    onChange={(event) => setSpecialtyName(event.target.value)}
                    placeholder="Digite sua especialidade, se quiser"
                    className={inputCls}
                  />
                )}
              </fieldset>
